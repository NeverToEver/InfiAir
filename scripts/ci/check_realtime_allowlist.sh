...
ALLOW = (
    ...
    ("csharp/godot/ProbeHost.cs", "Time.GetTicksMsec", 4,
     "启动耗时**观测**读数（2 处：装载完成、首帧含绘制；`--startup-time` 用，分段是为了判巨帧"
     "归装载还是归着色器编译——合成一个总耗时读不出归属）与返航探针的环境前置守卫（2 处：记墙钟起点、"
     "算已耗真实毫秒）——守卫用途是「若 90 帧耗时已达生产宽限则无法判别时间基准，显式报错」，"
     "度量的是真实耗时，非判定输入；探针的判定本身只用帧数与过场状态"),
