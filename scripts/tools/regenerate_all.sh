#!/usr/bin/env bash
# 统一素材生成入口:依固定顺序重跑 9 个离线生成器 + SFX 微调写回,输出与仓库提交资产一致。
...
echo "==> [1/9] 玩家战机 + 受击帧贴图 (generate_player_sprite.py)"
...
echo "==> [2/9] 初始机型五套玩家机贴图 (generate_machine_sprites.py)"
# 骨架（九锚点 + 挂点）与标准型一致,只换剪影与细节;落盘前有锚点自检,失败即整脚本失败
"$PY" "$SCRIPT_DIR/generate_machine_sprites.py"
echo "    产物: assets/sprites/player_ship_{peregrine,sledge,repeater,bulwark,colossus}.png"
echo "          + 每型 _glow.png(能量发光遮罩)/ _hit_1.png / _hit_2.png,共 20 张"

echo "==> [3/9] 敌机/精英/Boss/航母/炮塔贴图 (generate_enemy_sprites.py)"
...
echo "==> [4/9] Boss P2 损伤帧 (generate_boss_p2_frames.py)"
...
echo "==> [9/9] 音效/BGM (generate_audio.py + tune_sfx.py --apply)"
