...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("# BALANCE_MAP — Value Location Map")
    lines.append("")
    lines.append("> Auto-generated by `python3 scripts/tools/gen_balance_map.py`; do not edit by hand.")
    lines.append("> Re-run the generator after adding/renaming value keys or adjusting cfg() calls.")
    lines.append("")
    lines.append("## How to change values")
    lines.append("")
    lines.append("- `data/balance.json` is the single source of runtime values; prefer editing it in the browser with `python3 scripts/tools/balance_editor.py` (change highlighting, type validation, auto backup).")
    lines.append("- Script-side `GameState.cfg(\"key path\", fallback)` only covers missing/corrupted json keys; per AGENTS.md, keep json and fallback values in sync when adding or adjusting values.")
    lines.append("- Cache values on hot `_process` paths once in `_ready()`/`_load_balance()`; do not query them per frame.")
    lines.append("")
    lines.append("## Static cfg() call sites (grouped by file)")
    lines.append("")
    cur_file = None
    for f, ln, key, default in static_calls:
        if f != cur_file:
            ...
            lines.append(f"### `{f}`")
            lines.append("")
            lines.append("| Line | json key path | Script fallback |")
            lines.append("| --- | --- | --- |")
        lines.append(f"| {ln} | `{key}` | `{default}` |")
    lines.append("")
    lines.append("## Dynamic concatenated key prefixes")
    lines.append("")
    for p in sorted(dynamic_prefixes):
        lines.append(f"- `{p}…`")
    lines.append("")
    lines.append("## Keys in json not statically referenced by scripts")
    lines.append("")
    lines.append("(Keys covered by dynamic keys or whole-section reads are omitted; manually judge whether the rest are dead keys)")
    lines.append("")
    for leaf in sorted(set(unreferenced)):
        lines.append(f"- `{leaf}`")
    lines.append("")
    lines.append("## Keys referenced by scripts but missing from json (fall back to default; add to json or confirm intentional)")
    lines.append("")
    seen = set()
    for f, ln, key, default in missing_in_json:
        if key in seen:
            continue
        seen.add(key)
        lines.append(f"- `{key}` (`{f}:{ln}`, fallback `{default}`)")
    lines.append("")

    OUT.write_text("\n".join(lines), encoding="utf-8")
    print(f"[gen_balance_map] {len(static_calls)} static calls, {len(dynamic_prefixes)} dynamic prefixes, "
          f"{len(set(unreferenced))} unreferenced keys, {len(seen)} missing keys → {OUT.relative_to(ROOT)}")
