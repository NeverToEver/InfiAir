...
func _refresh_texts() -> void:
	...
	# E02/G03：通关后或有进行中存档时禁用教程按钮——重进教程 tutorial._ready 会无条件
	# delete_save()，静默删掉进行中存档；禁用 + _on_tutorial_pressed 守卫双保险
	_tutorial_button.disabled = GameState.tutorial_done or GameState.has_save()
...
func _on_tutorial_pressed() -> void:
	# E02/G03：通关后或存在进行中存档时禁入教程（UI 已禁用按钮，此处兜底防键盘/程序化调用）——
	# 重进会触发 tutorial._ready 的 delete_save()，静默删掉进行中存档
	if GameState.tutorial_done or GameState.has_save():
		return
	get_tree().paused = false
