...
- **FUI 仪表细节**：环带外圈 72 档仪表刻度环（每 6 档长刻度）+ 卡间隙刻度镜像到内缘；内域仪表簇（菱形准星 + 「NN/NN」序号读出 + 层深小格 + 细线，挂在可视内域弧中点下方、避开返回芯片位——圆心锚在屏外，画在圆心处不可见）+ 轮毂径向辉光贴图（`GradientTexture2D` 运行时生成、半径探进可视透镜）+ 内域雷达扫掠双扇环（活性态限定）；读出字符串只在聚焦/项数/层深变化帧重建（热路径零分配）
- **卡片与反馈**：聚焦卡四角 L 形锁定托架（随悬停过渡因子由外滑入 + 呼吸）、底板投影随悬停浮起拉长、顶缘受光高光线、图标槽聚焦内辉光；聚焦变化在环带外缘打锁定 ping 弧（移焦/换层反馈，随 `FocusChanged` 触发）
- **屏幕取景框**：`RadialMenuLayer` 新增四角 L 形 HUD 括弧（暗托底双描保证压在宿主/HUD 面板上仍可辨）+ 上/下/右三缘中点刻度（左缘不设——轮盘整圆永远叠压左缘中点；错峰绘入/收起、随开合编排、`RaiseWheel` 保持最上）；welcome 补 `RaiseWheel`（主区面板后入树此前会盖住括弧）
- **验证**：build 0w/0e + xUnit 149/149 + format 三 csproj 零 diff + import 零警告 + 300 帧 0 错误 + smoke 0 FAIL + 窗口化截图 judge 三轮（修复 仪表簇画在屏外圆心不可见、括弧 alpha 0.32 无托底整屏缩放下不可辨、boot 截图改帧计数+热身避窗口首帧黑帧、截图工具根层未用宿主槽距 8 卡溢出、welcome 括弧被主区面板盖住）；设置/基地页聚焦项与右面板初始章节不同步属宿主页既有接线，登记 ROADMAP
- **截图工具**：`radial_wheel_capture` 改四态——全容开机中段/全容根层（宿主页同款 `SlotAngleFor` 槽距，帧计数等待避开窗口启动黑帧）+ 溢出滚动/下钻（默认槽距演示溢出语义）

### 优化（2026-09-08，圆盘轮盘性能/视觉/空间链路三阶段升级）

- **性能（分层重绘 + 空闲快路径）**：轮盘即时绘制拆 钢构/卡片/特效 三层 `RadialWheelLayer` 子层按重绘频率独立重铺——悬停/滚动只重铺卡片层、确认特效只重铺特效层，环带/面包屑/轮毂在层深与收缩缩放不变时零重绘；无动画且输入静默（鼠标未动/无按压，活性余温 2.5s）时零重绘零变换写入，`_rescan` 兜底吸附/收缩/装载后「内容变了鼠标没动」的重扫；大弧段数 128→96/96→64
- **视觉**：修复 图标槽错画在卡片原点压住标签的结构性 bug（槽框与字形现同变换绘制于槽中心）；卡片悬停/聚焦改逐卡平滑过渡因子（渐亮渐浮起代替瞬时跳变）+ 左缘选择轨 + 聚焦尖端呼吸标记 + 端点倾角钳 ±28°→±20°（可读性）；环带改径向明暗渐变（内背光外受光）+ 卡间隙刻度；轮毂增刻度环与活性慢旋双弧；确认特效改旋转扫掠扇形 + 三级衰减波纹；活体动效（呼吸/慢旋）限定活性态，空闲冻结零开销
- **空间链路**：`RadialMenuLayer` 新增聚焦引线——轮盘聚焦卡与右区内容面板连成 HUD 引线（正交三段走线 + 端点菱形 + 聚焦脉冲；画布坐标直绘 + 运动门控重绘，不随 holder 入场滑动漂移；锚点无水平走线空间时守卫放弃）；`RadialWheel` 新增 `FocusChanged`/`FocusedTipLocal`/`IsBusy` 联动 API；暂停页聚焦说明卡接 `FocusChanged` 实时跟随（补齐原注释承诺的联动）；欢迎主区/设置/结算/基地当前目录面板接引线锚点；页面隐藏经 `VisibilityChanged` 自动断供轮盘（防隐形吞命中区输入）
- **审核修复**：活性环 `_spinA` 衰减移出扫描 gate（否则脱离活性后快路径拦截帧让渐隐永不发生，留冻结幽灵弧）；`Load` 初始化 `_lastFocusIdx` 消除开页合成 `FocusChanged`（避免脉冲 tween 击穿引线延迟淡入）；`FocusedTipLocal` 纯查询化（去 `Array.Resize` 副作用）并补 pop 缩放口径
- **验证**：build 0w/0e + xUnit 148/148 + format 零 diff + import 0 警告 + 300 帧 0 错误 + smoke 152 项 0 FAIL + 窗口化实机过目（轮盘三态 + 暂停/结算引线走线逐像素核对）
...
### 架构（2026-08-11，第六轮：GameState 拆域收官——SettingsService + 保留分支合并，全量行为零变化）
...
### 架构（2026-08-11，第七轮：GameState 拆域收官——InputBindingsService + UserSessionService，全量行为零变化）
