...
- **母舰与黎明站** — 蓄力召唤母舰随行支援：穿梭门在落点张开、母舰破门入场（这 0.8 秒不夺操作），到位后牵引回收进入火力平台；返回黎明站休整：战机库、维修补给、路线契约、任务规划，用 RP 换资源后继续出击。
...
标题屏：**任意键**新的一局 · **C** 继续上次出击 · **T** 教程（进度按阶段续接，中途退出后入口提示变「继续教程」）· **P** 练习（直选 Boss 型别 / 起始难度 / 遭遇事件，不计进度、不写存档）。全站菜单为左缘圆盘 UI（标题屏的两个入口与 T/P 快捷键并存），方向键 / 摇杆旋转、确认键按下，两个入口也可用手柄聚焦后按 A 确认。键鼠与手柄为唯二输入面（PC 专用，触屏已退役）。
