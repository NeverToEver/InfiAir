...
标题屏：**任意键**新的一局 · **C** 继续上次出击 · **T** 教程（进度按阶段续接，中途退出后入口提示变「继续教程」）· **P** 练习（直选 Boss 型别 / 起始难度 / 遭遇事件，不计进度、不写存档）· **M** 机型（六选一：标准型 + 游隼 / 重锤 / 连弩 / 壁垒 / 巨像，各一项加成与一项代价、各一副外观；六型在飞行手感上也各有性格（残影 / 曳光 / 后坐 / 受击读数等表现差异，判定与数值不变）；轮盘转动时旧机飞出、新机飞入并演一段该型的特性，回车 / A 或点机体才算选定，选定即存、新的一局生效）。全站菜单（暂停 / 结算 / 基地 / 设置 / 机型）为左缘圆盘 UI：方向键 / 摇杆旋转、确认键按下。标题屏本身不入圆盘——底部「教程 / 练习 / 机型」是可聚焦按钮（手柄方向键 / 摇杆移焦点、A 确认），**T** / **P** / **M** 与鼠标点击照旧。键鼠与手柄为唯二输入面（PC 专用，触屏已退役）。
...
- `best.json` — 跨局的局末结果记录（存活时长 / Boss 击杀 / 最高难度档 / 目标达成）；与检查点分区，死亡删档不影响它。
