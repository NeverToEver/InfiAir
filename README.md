...
- **可自定义准星** — 四形状（括角 / 经典十字 / 圆环 / 净点）+ 大小 / 粗细 / 间隙 / 透明度 / 旋转 / 中心点 / 描边 / 自由取色；多档案簿与 `INF1-` 准星码跨设备复刻。准星盖住敌机即变色、进入强追踪时转一圈收拢成锁定框。
- **Boss 轮换与遭遇事件** — 4 种 Boss 阶段弹幕 + 狂暴，50 秒打不死就逃逸；精英炮塔、编队突袭（炸弹可击落、可弹反打回）与四种干扰雾事件穿插出现（遭遇事件之间按优先级互斥，迷雾独立掷签、可与遭遇并行但组内只出一个）。
- **母舰与黎明站** — 蓄力召唤母舰随行支援：穿梭门在落点张开、母舰破门入场（这 0.8 秒不夺操作），到位后牵引回收进保护舱；返回黎明站休整：战机库、维修补给、路线契约、任务规划，用 RP 换资源后继续出击。
- **战术琥珀视觉** — 暖炭黑底、琥珀主交互，金属 / 全息线光 / 文字灰全部收在同族暖偏（冷青已退役）；世界层手写屏幕后处理（辉光 / 调色 / 晕影 / 颗粒）补足 GL Compatibility；重伤时屏幕碎裂、呼吸与心跳随血量下沉，可一键减少闪光。
- **窗口与性能** — 窗口化 / 无边框全屏；渲染分辨率五档（720p / 900p / 1080p / 1440p / 4K，按显示器列档），窗口自由拖拽并记忆任意尺寸；视角三档缩放；帧率上限九档（30 / 45 / 60 / 120 / 144 / 165 / 180 / 240 / 不限制）+ 垂直同步，设置页实时显示当前帧率与垂直同步实际状态。
- **输入可调** — 键鼠与手柄两路并行：键盘动作除开火 / 天赋面板 / 暂停 / 重开外均可改键（冲突键自动让位、可恢复默认），手柄按 SDL 标准位置装配并自动区分 Xbox / PS 标签；右摇杆灵敏度与死区、开火按住或切换、Ctrl/Shift 按住或切换都在设置页。
- **音量与无障碍** — 主音量 / 音乐 / 音效三分路独立调节；减少闪光（削弱受击闪屏与心跳脉冲）、屏幕震动强度、命中顿帧强度与动效强度都可调（后三者可完全关闭）、高对比弹体给敌弹加亮色描边（默认开）；设置可一键恢复默认。
...
标题屏：**任意键**新的一局 · **C** 继续上次出击 · **T** 教程（进度按阶段续接，中途退出后入口提示变「继续教程」）· **P** 练习（直选 Boss 型别 / 起始难度 / 遭遇事件，不计进度、不写存档）· **M** 机型（六选一：标准型 + 游隼 / 重锤 / 连弩 / 壁垒 / 巨像，各一项数值加成与一项代价、各一副外观；六型在飞行手感与能力强度上也各有性格——残影 / 曳光 / 后坐 / 受击读数等表现差异，外加弹反窗 / 弹反循环 / 冲刺冷却 / 加速续航各一强一弱，标准型是全部基准；轮盘转动时旧机飞出、新机飞入并演一段该型的特性，回车 / A 或点机体才算选定，选定即存、新的一局生效）。全站菜单（暂停 / 结算 / 基地 / 设置 / 机型）为左缘圆盘 UI：方向键 / 摇杆旋转、确认键按下。标题屏本身不入圆盘——底部「教程 / 练习 / 机型」是可聚焦按钮（手柄方向键 / 摇杆移焦点、A 确认），**T** / **P** / **M** 与鼠标点击照旧。键鼠与手柄为唯二输入面（PC 专用，触屏已退役）。
...
- `best.json` — 跨局的局末结果记录（存活时长 / Boss 击杀 / 最高难度档 / 目标达成，另存本局机型作平衡回流、不上屏）；与检查点分区，死亡删档不影响它。
- `settings.json` — 语言、难度、窗口与分辨率、帧率上限、键位、准星档案、辅助选项等全部设置；删除即恢复默认。
...
- **不想被闪烁影响**：设置页的「减少闪光」开关减弱受击闪屏与屏上脉冲（音效保留）。
...
- **数值调参只改 `data/balance.json`**（可视化编辑：`python3 scripts/tools/balance_editor.py`，仅依赖 Python 标准库）。编辑器按 `scripts/tools/balance_meta.json` 标出每个键的中文说明、单位与取值范围（越界标红并说明代码会怎么处置），带搜索过滤、撤销、改动清单（可复制成 Markdown 表格）与备份回滚；「快速预设」（`balance_presets.json`：轻松一点 / 新手辅助 / 减少干扰 / 快节奏 / 硬核）给不熟悉单个键的人一个按感受选的入口，套用是相对调整、可先看改动清单、一次 Ctrl+Z 全退；「平衡分析」页给出难度曲线、战斗节奏、经济与进度、增幅每层收益与取值体检，每条公式标了 C# 出处；「设置」页做精细数值操作（直接编辑原始 JSON，应用前过与保存同一套结构校验）、导入导出与「恢复已提交版本」。服务是临时的——**关掉页面即自动退出**，闲置超时（默认 30 分钟）也会退出。`--readonly` 只读，`--balance <路径>` 指向副本，`--idle-timeout` 改闲置分钟数。**游戏内也留了入口**：设置页「辅助与关于」→ 数值管理器（在跑就打开、没跑就启动；仅源码运行可用，导出包不含该工具）。改完数值**回到标题屏即生效**——本局参数在开局读入，局内不重载，所以调完回一次标题屏再开下一局。**UI 调色板只改 `csharp/godot/UITheme.cs`**（场景文件内嵌的 VFX/粒子配色以对应 `.tscn` 为准）。
...
- **改什么读哪份**：见 [AGENTS.md §1 路由表](AGENTS.md)——设计定稿 [DESIGN_BASELINE](docs/DESIGN_BASELINE.md)、方向 / 债务 / 决策 [ROADMAP](docs/ROADMAP.md)、外部依据与出处 [REFERENCES](docs/REFERENCES.md)、发布说明 [RELEASE_NOTES](docs/RELEASE_NOTES.md)。
