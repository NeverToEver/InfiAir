...
发布页提供 Linux / Windows 预构建包（x86_64，单文件可执行，含安装/卸载脚本）：

- **Linux**：解压后运行 `./install.sh`（用户态安装到 `~/.local/share/infiair`，创建命令入口 `~/.local/bin/infiair` 与桌面菜单项）；`./uninstall.sh` 卸载（加 `--purge` 同时删除存档与设置）。也可以不安装，直接运行包内的 `InfiAir.x86_64`。
- **Windows**：解压后运行 `install.bat`（安装到 `%LOCALAPPDATA%\InfiAir` 并创建开始菜单快捷方式）；`uninstall.bat` 卸载（加 `/purge` 同时删除存档与设置）。也可以不安装，直接运行 `InfiAir.exe`。
