...
- 敌机 4 种移动模式（直下 / 正弦 / 折线 / 俯冲），约 1/3 会朝玩家开火；精英敌机 HP 更高、分数更高，随分数混入。
- 每 500 分触发一次 Buff 三选一（暂停游戏）：强力射击（伤害+1，可叠）、急速射击（射速+25%，可叠）、散射弹道（+1 弹道，最多 3 层）、额外生命（+1 命）、自我修复（每 2s 回 0.5 命，可叠）。
