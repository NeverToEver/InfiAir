...
- **音量与无障碍** — 主音量 / 音乐 / 音效三分路独立调节；减少闪光（削弱受击闪屏与心跳脉冲）、屏幕震动强度与动效强度（后两者可完全关闭）可按需调整，设置可一键恢复默认。
...
标题屏：**任意键**新的一局 · **C** 继续上次出击 · **T** 教程（进度按阶段续接，中途退出后提示变「继续教程」）· **P** 练习（直选 Boss 型别 / 起始难度 / 遭遇事件，不计进度、不写存档）。菜单为左缘圆盘 UI，方向键 / 摇杆旋转、确认键按下。键鼠与手柄为唯二输入面（PC 专用，触屏已退役）。
