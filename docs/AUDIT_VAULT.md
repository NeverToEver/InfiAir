...
# AB 系列（2026-08-11，第八轮：全量对抗性复审——发现登记，修复待批次）

- **审计**：8 路并行只读对抗性复审（2026-08-11 连击/保底新代码、Boss 族、玩家/战斗核心、敌机/生成/母舰、事件系统、数据/持久化层、UI/导航/结算、服务/框架/演出，全部 177 个生产 C# 文件无重叠覆盖），核心策略=追查既有修复（K02/H06/R06/C16/L06/Z 批次/AA 系列）的**孪生遗漏路径** + 文档×代码×git 三角核对；各分区 ~50 项候选经自我证伪后剔除（附证伪记录），仅上报证据链完整的发现。基线 `dotnet build` 0w/0e + xUnit 111/111。**13/23 项已由协调人亲自读码复核确认**。无 P0；正常游玩路径可达仅 1 项（AB1）。
- **发现（23 项）**：
  - **P1 ×1**：**AB1** `LaserWeapon.cs:228-238` × `Player.cs:946,1024` × `Main.cs:1019-1023` 激光 active 期间返航 → 自动开火永久禁用（K02/H06 只修「入场期不触发」，未修「入场期 active 光束自然结束」孪生路径：树暂停冻结激光 active → 恢复时 PlayEntryAnimation 捕获 `_entryPrevAutoFire=false` → EndBeam 恢复 true → FinishEntry 覆盖回 false → 自维持哑火至本局结束，无恢复路径）。
  - **P2 ×15**：**AB2** `BuffSelect.cs:161-168` dynamic_weight 四子键裸读无判型（`"enabled":"false"` 字符串误读为开——C16 存档侧同款陷阱；坏 hp_ratio → 保底+加权静默失效；ids 缺键回退空数组与同函数默认值口径矛盾）；**AB3** `Boss.cs:965,1024` 模式表 interval 无下限（≤0 每物理帧攻击，R06 只封 enrage interval 族）；**AB4** `Boss.cs:534` E2PointCount 无下限（0 → 二型狂暴 ACTIVE 冻结 6s，同族 count 键均有 floor）；**AB5** `Spawner.cs:168` elite_wave_size 无下限（0/负 → 精英波静默吞特殊槽，:257 WaveSize 有 Max(1)）；**AB6** `Spawner.cs:143-144,558` boss_time_limit/boss_min_interval 无下限（≤0 → Boss 无限连出）；**AB7** `Mothership.cs:238,633-636` mag_cell_time 无下限（≤0 每帧耗 1 格、STAY 驻留瞬结；mag_cells 同批已钳孪生遗漏）；**AB8** `Spawner.cs:241-250` MergeType 数值域未校验（仅判型：fire_interval 0 → 机枪化、score 负 → 倒扣分、radius 0 → 直穿）；**AB9** `GameEventManager.cs:670,712` fog weights/durations 条目值未判型（坏值 InvalidCastException 每 check_interval 崩溃；2026-08-10 批次只修遭遇事件）；**AB10** `GameEventManager.cs:193-197,636-640` 场景重入残留遭遇活跃态 → 幽灵 EventEnded（无 started 的 ended）+ 首帧 ForceTrigger 被拒（消费方仅测试，危害低）；**AB11** `UserDb.cs:324-334` DeleteUser 回滚不覆盖文件副作用——存档文件在 users.json 提交前已删，落盘失败时「删除失败」但对局存档已永久销毁，回归测试只断言内存回滚（盲区）；**AB12** `ProgressionCurves.cs:122` × `GameState.Save.cs:146` 存档 elapsed 无上界（SaveNum 仅判型）——手改 1e300 → `(long)` 未定义转换得 long.MinValue → 难度乘数 -1.4e19 → 敌 HP/伤害/速度 ramp 全反向 + 刷怪风暴，击穿「损坏回退默认」与「难度单调不减」双防线；**AB13** `ExitConfirm.cs:115-124` × `PauseUi.cs:200-205` 退出确认 0.3s 淡出窗口未屏蔽 R 键——确认后按 R → ReloadCurrentScene 杀 tween → Quit 永不执行，但 battle 模式 DeleteSave 已执行（档删、未退出、静默重开新局）；**AB14** `GameState.Save.cs:218` × `Main.cs:163` 继续对局恢复 touch_controls 不广播 TouchControlsChanged（Main._Ready 先 SetEnabled 后 ApplyRunSave，恢复值不回流，触屏控件与设置脱钩）；**AB15** `GameState.Difficulty.cs:120` SpreadEnemyCap 裸 `(int)` 无上界（>2^31 回绕负 → spread 弹幕敌机整类消失；ScoreMultiplier 同文件已钳）；**AB16** `Starfield.cs:50-53,79-93` far_count/near_count 只判型+非负无上界（手改 1e9 → ~24GB 数组启动即 OOM）；**AB17** `GameState.Save.cs:570` 登录用户榜单显示裸 `(int)` 截断（本地榜经 SaveInt 钳制，user_db 榜孪生遗漏 → 负数名次分）。
  - **P3 ×7**：**AB18** `Boss.cs:397-399` P2/ENRAGE 血量比例不校验顺序（倒挂跳过 P2 段直接狂暴，违 BOSS_REDESIGN §4.1）；**AB19** `Enemy.cs:822-825` 离场方向注释「就近侧方」与代码相反（迁移保留失实注释，行为与旧版一致）；**AB20** `GameEventManager.cs:514` × `Spawner.cs:558` autoload 帧序在前 → 同帧 Boss/遭遇竞态优先级反转（违 ELITE_TURRET_EVENT §6.3「same-frame race → boss wins」）；**AB21** `BaseConsole.cs:695`、`SettingsUi.cs:776` 类尾悬挂孤儿 `/// <summary>` 片段（M5 迁移残留）；**AB22** `Hud.cs:151,834` × `SegmentedBar.cs:161-226` hud.boss_bar_segments 配置恒无效果（加权分支只迭代固定 3 段）；**AB23** `SettingsUi.cs:536` 键盘调整摇杆滑杆不持久化（仅 DragEnded 写盘；正常退出兜底，仅异常终止丢失）。
- **核心模式**：约 10/23 项为既有修复的**孪生遗漏**（每轮只封一侧入口）——建议后续修复批次做「钳制族/判型族逐键清单化」一次收口，而非逐轮补漏。2026-08-11 combo/保底新代码本身质量高（断连同源、奖励路径、溢出边界、保底抽取全部核实无误），唯一缺口 AB2。
- **修复建议优先级**：① AB1（P1 正常可达）；② AB13/AB11/AB12（数据安全）；③ AB16/AB15/AB17/AB14（判型/钳制族收口）；④ AB2/AB3/AB5/AB6/AB7/AB8/AB9/AB4（钳制族清单化）；⑤ AB18-AB23（P3 批次）。
- **状态**：✅ 全部修复（2026-08-11 全量落地，4 批提交 + 模拟 CI 全绿；验证口径见下）。

## AB 系列修复起效记录（2026-08-11 全量落地，4 批提交 + 模拟 CI 全绿）

> 执行依据：`docs/archive/2026-08-11-ab-series-fix-guide.md`（批次 1-4）；验证口径：`dotnet build` 0w/0e、
> xUnit 113/113（含新增 AB11 文件幸存 / AB12 巨值钳制 / AB17 榜单钳制用例）、三工程 format 零 diff、
> `godot --headless --import` 0 引擎错误、main smoke 300 帧 0 错误、全量断言场景 0 FAIL、
> BALANCE_MAP 重跑零 diff（AB22 删键后同步）。各项「当前默认值行为零变化」均已核实。

- **批次 1（P1，AB1）**：改了什么——`Player.cs` 新增 `OverrideEntryAutoFire(bool)`（`_entryPhase != 0` 时覆盖 `_entryPrevAutoFire`），`LaserWeapon.EndBeam()` 恢复 autofire 后同步调用。为什么起效——入场期间 EndBeam 恢复的 true 不再被 FinishEntry 无条件踩回 false，返航冻结激光 active 的哑火路径闭合。如何验证——build 0w/0e；smoke + 全量断言场景回归；返航→继续出击场景覆盖（激光 buff 无移除机制，仅手动路径可达，未新加断言场景）。
