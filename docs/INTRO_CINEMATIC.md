...
| --- | --- | --- | --- | --- | --- |
| 1 | 远景推近 | 2.8s | 超巨型环形空间站深空爆炸，镜头极远景缓慢匀速推近；火光暗红→刺目橙白；碎片与尘埃前景层次；密集结构细节与破损舱段；深邃星空+远处星云；冷底暖高光 | 复用 Starfield 做星空底；环形站用多段 Polygon2D 弧段+舱段矩形拼装；整体容器 scale 0.7→1.0 匀速推近；爆炸核心=叠加态圆形辉光 modulate 从暗红(0.5,0.1,0.05)升到橙白(1,0.9,0.7)并放大；GPUParticles2D 两组（碎片=小矩形高速外抛带旋转，尘埃=慢速大颗粒低透明度前景） | 爆炸 SFX（既有 `GameState.SFX_*` 爆炸音） |
| 2 | X光链式爆炸 | 2.5s | 空间站内部蓝白 X 光透视；橙红能量流沿舱室走廊/管道线性链式传导、逐层引爆；多层甲板/支撑骨架/舱室隔断层级清晰；冷蓝×橙红双色对比 | 冷蓝底(0.02,0.05,0.12)；Line2D 绘制站体剖面线框（3-4 层甲板横线+骨架竖线+隔断）；链式路径=预设折线，用 Tween 驱动 `Line2D` 逐点亮起（橙红、宽度渐粗）；路径节点处依次爆开橙色圆闪（scale 0→1→淡出），节奏 0.2s/节点 | 短促爆炸音×3 连发 |
| 3 | 驾驶员冲刺 | 2.5s | 轻型飞行服驾驶员在狭窄弹射准备舱全速冲刺，姿态前倾手臂大幅摆动；红色应急灯高频闪烁；墙壁管道蒸汽泄漏；地面黄色警示标线；体积光、动态模糊 | 侧视走廊：天花板/地面透视线+黄色警示条纹（向反方向滚动表现冲刺）；驾驶员=深色剪影多边形小人（躯干前倾+双腿/双臂用 rotation 正弦摆动模拟奔跑循环）；红色全屏 ColorRect 以 6Hz 正弦呼吸闪烁；蒸汽=白色半透明粒子上飘；速度线若干 | 警报感可用既有命中音低频重复（可选，阶段2再定） |
| 4 | 操作台紧急启动 | 2.5s | 驾驶舱内，双手在按钮/触控屏控制面板快速精准操作；屏幕红色倒计时与警告 UI；随后双手猛抓两侧金属把手、指节发白、身体 anticipating 后仰；仪表灯光映面 | 驾驶舱前景框架（深色舱壁多边形容器）；控制台=矩形面板+网格小方块随机高频亮灭（按钮操作感）；主屏=红底 Label 倒计时 3→2→1 + tr("INTRO_WARNING") 警告行闪烁；两侧金属把手（圆角矩形）；结尾 0.5s 整体轻微后仰（rotation -2°）+ 屏幕白光渐强 | 倒计时结束接镜头5引擎音 |
| 5 | 弹射尾追视角 | 2.8s | 战机正后方尾部视角，从弹射轨道高速射出；机身两侧橙红火焰舌舔舐未吞噬；强动态模糊与镜头震动；轨道金属结构向画面边缘飞速后退；冷底暖焰对比 | 复用玩家机贴图（尾部视角=原 sprite 旋转180°缩 1.4 倍置于画面中心偏下）；弹射轨道=两侧多组横档线以加速度向画面边缘飞出；火焰=左右两道 GPUParticles2D（橙红、舌状 lifespan 短、贴机身两侧）；全屏速度线；Camera2D 无（用容器 offset 随机震动模拟，±6px） | 引擎/加速音（既有 SFX） |
| 6 | 远景收束 | 3.0s | 史诗远景：左下燃烧残骸废墟，中央偏右战机加速驶离，远处补给舰编队光点阵列同向；壮丽星云与恒星背光勾勒剪影；三分法构图；静谧与毁灭并存 | 星云=2-3 个大半径低透明度叠加圆（紫/蓝，缓慢异向漂移）；恒星背光=右上强辉光点+横向 anamorphic 光晕（宽蓝白亮条脉动）；残骸=左下深色不规则多边形剪影+橙色余烬闪烁点；战机=小剪影向右上加速移动（ease_in）；补给舰编队=5-7 个小光点阵列同向跟随（各带引擎拖尾短线）；最后 0.7s 全屏淡黑衔接标题定格 | 无（静音收束，由对局 BGM 接管） |
...
| `data/translations.csv` | 新增键：`INTRO_SKIP`（按 Esc 跳过 / Esc to Skip）、`INTRO_WARNING`（警告 / WARNING）、`INTRO_SUB_1..6`（六镜头叙事字幕，中英双语） |
...
- [x] 窗口模式每镜头至少 1 张截图人工核对（/tmp/intro_shot1..6.png，2026-07-27 核对：6 镜头主体元素/相机运动/配色基调均到位，无大面积空白、无元素堆叠、无文字溢出；镜头 5 轨道横档「梯子感」偏重、镜头 6 战机为彩色贴图而非剪影，留阶段 2 精修）
