...
- 范围区间：**48 ~ 112 HP**（四型分别为 65-72 / 48-56 / 95-112 / 56-66，2026-08-02 按 balance.json 重采样）
- 典型值：**约 65-72 HP**（最常见的首型区间中值）
- 难度系数：`difficulty.hp` 易 ×0.75 / 中 ×1.0 / 难 ×1.5

（参考：精英 135-210；Boss 800 × hp_mults。本事件炮台按“普通单位”量级取血。）
...
t=2.0 → 3.5s   基座盖板旋开，炮塔升起充能（不可被攻击：monitorable=false）
