...
- **已知覆盖缺口（人工项）**：返航过场跳过宽限当前无自动化回归面（原 `ReturnCinematicTest` 随测试资产退役删除），改其基准只能靠人工过目「实战按住方向键/空格进入返航，1.2s 内按键不跳过、之后可跳过」。要做到可自动化，应加一个走 `DoSkip(false)` 宽限边界的无头探针，而不是靠换时钟规避。
