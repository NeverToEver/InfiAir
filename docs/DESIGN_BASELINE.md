...
2. **全屏周期脉冲频率**按 §1.9.1 登记 `core FlashBudget`（<3Hz、减闪归零）；新增行的频率取自 balance 的 BPM 换算，改 BPM 越阈值即单测判红。
...
**动效强度（设置项，2026-09-18 人类定向新增）**：键 `fx_intensity`（settings.json，0..1，默认 **1.0**，步进 5%），落设置页「辅助与关于」页，形态与屏幕震动强度/命中顿帧强度同族（0% 时文案明示「已关闭」）。语义＝**缩放本轮新增动效的振幅，不改频率**；取值口与钳制单源在设置服务，消费方一律经它取，不在各处留本地系数。默认值与各动效振幅属玩家可感取值，登记在 `docs/ROADMAP.md` 待确认。

**数值与单源**：新增数值一律落 `data/balance.json effects.motion.*`（呼吸频率上下限、呼吸与拍点振幅、三档 BPM、流光与速度线时长等）；节拍相位、拍点包络与呼吸频率映射等算式下沉 `csharp/core/Visual/` 并配单测。

**本轮落地清单（六批，各批独立可回退）**：① 节奏底座（core `Visual/Rhythm` + `FlashBudget` 全屏呼吸行 + 设置项 `fx_intensity`）；② 节奏服务与全屏呼吸（`VisualRhythm` 取样音乐播放位置、`world_grade` 单点实现）；③ 敌弹尾部能量动效（共享 `bullet_energy.gdshader`，只作用于低 alpha 尾段，弹体与判定逐位不变）；④ 舰体事件流光（擦弹 / 弹反 / 受击 / 连击里程碑各扫一次，经 `FlashBudget.AllowsOneShot`）、敌机损伤分级（能量层乘子，与受击白闪通路不相交）、入场落位与编队相位波；⑤ 背景战况响应（事件驱动速度线、星野滚动速度随难度、行星自转与微视差）；⑥ 界面互动性（设置页控件收归 `UITheme` 皮肤、UI 音效族、HUD 数值滚动）。**B6「打击感分层」不单独成批**：普通/精英/Boss 的击毁分层在 §2.7/§2.8 已存在，连击里程碑的非数字反馈并入第 ④ 项。
