...
- **不做的**：机型专属武器形态（Cave 式 A/B/C 差异）、机型解锁与升级、机型影响得分。
  （把机型性格从数值扩到机体表现 / 手感层已落地为 §1.18 手感档案；**玩法层**差异仍是冻结项，
  重启须先出决策。）
...
机型差异从「五条乘区 + 外观」扩到**飞行中的机体读得出性格**。三层里只落 ①表现层与 ②手感层：
**判定几何（碰撞圆 / 擦弹环 / 弹道）、玩法数值（五条乘区 / 敌弹 / 刷怪）逐位不动**；
③玩法层（冲刺撞伤 / 击退 / 破甲等）仍冻结，重启须先出决策。
对照基准是标准型（档案全基准：既有玩家的旧画面逐位一致），差异小到读不出＝白做、
大到能换胜负＝隐形收益，两层都算失败。
