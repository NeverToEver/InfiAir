...
### 2.14 输入回应层（2026-09-19 追加，交互性批次）

继 §2.7–§2.13 五轮之后的**第六轮**，主题是**输入回应**：前几轮做的是「世界发生了什么」的反馈（命中/击杀/爆炸/事件都有成套回应），本轮补「玩家自己的请求被系统拒绝时」的回应——行业口径里「命中无声（silent actions）」与「丢按键」是手感杀手（出处见 `docs/REFERENCES.md` §4.8）。仍为纯表现与输入面改善、**玩法判定零改动**：输入缓冲只把「就绪前一小窗内的按下」延到就绪帧生效，不改冷却/燃料/伤害任何数值；拒绝回应不改变拒绝本身。

**三批落地**：

① **命中与弹反确认音**：玩家弹命中敌机补轻量金属 tick——命中是全项目频次最高的交互，此前只有击杀有爆炸音，「命中无声」正是行业口径的头号静默面；弹反成功由借用冲刺音改用专属确认音（本作唯一防御机制，音色须与「移动」区分）。新增两枚纯合成音效（零随机流，`scripts/tools/generate_audio.py` 既有产物逐字节不变的可复现不变式照旧）；`SfxId` 末尾追加 `Hit` / `ParrySuccess`（五张表下标对齐，插队即错位）；命中音的最小触发间隔（45ms）与复音上限（2）防弹幕下逐帧刷屏，拦截在 SfxPlayer 目录表。

② **动作拒绝回应**：弹反/冲刺在冷却中或燃料不足时按下，此前输入被静默丢弃。现在：对应能力槽播一次**否认脉冲**（一圈**向内收拢**的危险色弧，与就绪脉冲的外扩互为反向手势；一次性闪光登记 `FlashBudget.OneShotFlashId.AbilityDenyPulse`，减少闪光下不播，抑制后环形冷却读数仍在）+ 低音 `UiDeny`；反馈经 Player 挂暂存标志、HUD 每帧轮询消费（与既有 HUD→Player 读数同向），不在 Player 直持 HUD 引用。

③ **输入缓冲**：冷却收尾 ≤`player.input_buffer_window`（**0.1s**）内的弹反/冲刺按下在就绪帧自动生效——「早了一点」的按键不丢（行业惯例窗口 <150ms）。判定单源 core `csharp/core/Combat/InputBuffer.cs`（单测钉住窗口边界/恰好就绪不缓冲/消费幂等/暂存过期）；**只缓冲时机类锁定（冷却），不缓冲资源类锁定**——燃料不足时按下即拒绝、不缓冲，燃料恢复不可预期，「燃料一到就自走一格」是意外行为。缓冲触发时重验燃料并取当前输入方向（零方向走准星回退，与直接按下同语义）。

**取值定稿**：缓冲窗口 0.1s、命中音最小间隔 45ms 与复音 2 为定稿取值（依据见 `docs/ROADMAP.md` Decisions 同日条目）；缓冲窗口改 `balance.json` 一处即调，音效限频改 SfxPlayer 目录表一行即调，否认脉冲时长为就地 const（§2.8 口径：小动效不新增 balance 键）。

### 2.15 全息面板投影感（2026-09-19 追加，虚影皮肤 2.0）

基地控制台「虚影皮肤」的全息面板从「半透明钢板」改为「投影光」语汇：行业口径里全息投影的读感是**半透明体积 + 像素/扫描质感 + 自发光**，是光不是拉丝金属（出处见 `docs/REFERENCES.md` §4.9）。纯外观批次——交互逻辑、页面结构、操作路径零改动。

**三个构成**：

① **发光边缘**：`ChamferedPanel` 新增 opt-in `HoloEdge`——切角轮廓双层外晕 + 内缘体积亮线；同时**退役钢板受光语汇**（顶缘受光带、底/侧阴影、拼板缝、暗色 keyline、铆钉——「被顶部光源打亮的金属板」与「自发光投影」语义冲突）。默认关，钢板面板（页壳/socket/弹仓）外观不变；切角点序趁机收敛回 `UITheme.FillChamferPoints` 单源（原 `ChamferedPanel` 自写一份点序）。

② **投影材质与扫掠**：`ApplyPhantomPanel` 关拉丝钢填充、开 HoloEdge 与内容裁剪（基地页板与母舰召唤小窗同族）；新控件 `HoloPanelFx`（替代 `BaseConsoleScanlines`）＝静态扫描线（每 4px 一条，无闪烁）+ 周期下扫柔边亮带（tween 只动 position/alpha，不触发重绘）。`HoloBoot` 叠一次性投影显影带（0.32s 自顶向底，与淡入同步）；分类图标收归 `Holo` 全息线光 token。

③ **减少闪光统一口径**：基地控制台既有装饰动效（标题数据抖动 3Hz、屏幕慢扫描带 8s）此前无视减少闪光设置，与母舰召唤小窗（停驻不闪）不一致——本轮统一：停播并复位静息态（抖动标签回全亮原位、慢扫描带收回屏外），恢复时整程重启不跳变；新增扫掠/显影带天生受控。可见性两层合成：页面切换是普通 Control 显隐（`HoloPanelFx` 自订阅 `VisibilityChanged` 随页启停），整层显隐是 CanvasLayer（不触发子节点可见性信号，宿主 `OnVisibleChangedForFx` 手工驱动）——非活跃时不留冻结的半程亮带。

**取值定稿**（依据＝行业惯例 + 实机截图比对，见 §2 决策权口径；改动位置＝`HoloPanelFx`/`ChamferedPanel` 常量一处）：

| 项 | 值 | 依据 | 代价 |
| --- | --- | --- | --- |
| 外晕双层 | 8px@14% + 4px@30% | 6px@10% 实机读不出「发光」，逐级加到该档最明显 | 边缘外溢 ~4px，面板间距 ≥4px 不蹭邻件 |
| 内缘体积线 | 22% alpha | 菲涅尔式内缘增亮，弱于核心线一档 | 无 |
| 常态扫掠带 | 峰值 10%、6s 一程 + 3s 空窗、渐变 0.9s | 慢到读作环境投影噪声而非事件；母舰小窗 5.5% 硬边带的柔化版 | 每页一条循环 tween（隐藏/抑制即杀） |
| 显影带 | 峰值 35%、0.32s、高钳 24–48px | 一次性事件须比常态亮一档才读得出「正在成形」 | 开页 0.32s 内压内容一次 |
| 常态带高 | 面板高 8%，钳 56–110px | 太窄读成硬线，太宽洗掉内容 | 无 |

### 2.16 机体活性与运动跟随（2026-09-19 追加，人类委托「战机动效太弱、没有互动感」）

继 §2.13（机体姿态）之后的**第六轮**，主题仍是「机体本身」：§2.13 补的是**事件性**姿态（侧倾/后坐力/推挤），本轮补**连续运动**中的活体感——机体对加速、转向、悬停、冲刺起手的连续反应。行业口径（出处见 `docs/REFERENCES.md` §4.10）：动画十二原则对程序化运动同样适用（squash & stretch / follow-through / timing）；Sakurai「宁可过火」「follow-through 决定印象」「idle 是基准不可晃」「响应优先于粘滞」。仍为纯表现层、**玩法判定零改动**：全部只写贴图节点的 Rotation/Position/Scale 与尾焰参数，机体根节点、判定几何、操控与弹道零改动——**视觉滞后绝不落在判定/操控上**（响应优先）。

**五项构成**（全部受动效强度缩放，0 ＝ 回到本批之前的画面；频率/速率不乘——缩振幅不改频率，§2.12 语义；算式单源 core `BodyPose` 并配单测）：
...
③ **悬停浮动**：贴图慢速微幅正弦沉浮（引擎悬浮感；幅度远小于滞后量级，守「idle 不可晃」）。
④ **冲刺弹跳**：冲刺起手贴图缩放 1→1+amp→1 抛物线过冲回拉（「过冲再回拉」；`BodyPose.PopScale`；渲染帧推进——物理早退的冲刺期也在走，缩放以 Init 捕获的设计缩放为基准倍增）。
⑤ **尾焰油门平滑**：三态档位（怠速/巡航/加速）切换由硬跳改指数平滑 + 升档瞬间短促增亮 kick（幅度/时长就地 const，§2.8 口径）；**降档不 kick**——收油门不该闪光；rate ≤ 0 或动效强度 0 直取目标（＝旧硬切换行为）。

**数值与单源**：新增数值落 `data/balance.json effects.motion.player_{lag,sway,bob,dash_pop,thruster}_*` 九键。

**取值定稿**（依据＝行业惯例（动画十二原则的程序化应用 + Sakurai Motion 系列 + §4.8 Swink 三层模型）+ 既有 §2.13 姿态键的量级锚定；人类可随时改判，改 `balance.json` 一处即调）：

| 项 | 值 | 依据 | 代价 |
| --- | --- | --- | --- |
| 滞后漂移 | 4px / 回中 10s⁻¹ | 与后坐力 2.5px 同量级再强一档（连续量 vs 事件量）；更快回中读作抖动 | 贴图中心偏离判定点 ≤4px；判定不动，擦弹/弹反读图不受影响 |
| 转向跟随 | 0.18rad / 回正 10s⁻¹ | 与侧倾 0.14rad 同量级再强一档；>0.2rad 会让人误读弹道方向 | 快速甩准星时枪口辉光（根节点挂点）与贴图短暂错位 ≤0.18rad，弹道判定不变 |
| 悬停浮动 | 1.6px / 0.6Hz | 幅度 < 滞后一半；频率贴着 §2.12 呼吸下限（0.5Hz）——「活着」但不成「晃」 | 无 |
| 冲刺弹跳 | +6% / 0.16s | 过冲量低于尾焰外层直径的视觉噪声线；时长约为后坐力 3τ（0.27s）的一半，短促 | 冲刺前 0.16s 贴图放大 ≤6%；残影首两帧继承（视觉一致） |
| 尾焰油门平滑 | 9s⁻¹ + kick 35%/0.15s | 收敛 ~0.25s 读作「拉动油门」而非迟滞；kick 借一次性事件提亮惯例 | 档位切换 ~0.25s 到目标态——尾焰是装饰层，不占 §4.8「绝不让玩家等」的输入面 |

### 2.17 机身反馈与损伤状态（2026-09-19 追加，机体活性二批）

继 §2.16（机体活性）之后的**同轮二批**：一批补的是「机体在动」的连续感（滞后/跟随/浮动/弹跳/油门），二批补**「机体在被驱动、在被伤害」的读数**——开火时机身被火光照亮、速度改变机身形变、中弹时机身被压扁、受损后冒烟、机动方向有喷口点火回应。行业口径见 `docs/REFERENCES.md` §4.11：RCS/微调推力器「按所需方向点火」（真实航天器惯例）；状态可读性（损伤状态要能被一眼读出，§4.7 已引）；muzzle flash 让每次射击「violent and immediate」（§4.8 已引）。仍为纯表现层、**玩法判定零改动**：全部只写贴图节点及其子节点的变换/调制与尾焰/烟粒子参数，机体根节点、判定几何、操控与弹道零改动。

**六项构成**（全部受动效强度缩放，0 ＝ 回到本批之前的画面）：
...
**数值与单源**：新增数值落 `data/balance.json effects.motion.player_{fire_light,stretch,hit_squash,sputter,damage_smoke,nozzle}_*` 十键；算式（速度伸缩的符号与钳制、交叉轴体积补偿、确定性喘振）单源 core `BodyPose` 并配单测（`StretchFactor` / `CounterScale` / `SputterFactor`；受击与开火的时间包络复用既有 `RecoilFactor`）。

**取值定稿**（依据＝§4.11 惯例 + 与既有 §2.13/§2.16 键的量级锚定；人类可随时改判，改 `balance.json` 一处即调）：

| 项 | 值 | 依据 | 代价 |
| --- | --- | --- | --- |
| 开火机身光 | +35% / τ0.07s | 亮一瞬即可读，与「最亮＝最危险」不冲突（局部、非危险色相、~0.2s 内衰减完） | 连发时机身维持微亮（7 发/s 下不到 3τ 即再次置位）——读数即「持续射击」 |
| 速度伸缩 | ±8% / 10s⁻¹ | 量级先按 §2.16 冲刺弹跳（+6%）略强——连续量压过一次性量才读得出 | 前飞时贴图较判定长 8%（判定不动）；残影继承（视觉一致） |
