...
- 设置项：难度、键位、语言、视角缩放、窗口尺寸、辅助瞄准档位、`reduce_flash`、音效/音量；语言切换经 `GameState.set_locale()`，UI 监听 `locale_changed` 刷新。
...
| 敌机生成路径 | 普通波次直接实例化 vs Boss-3 小怪走对象池，**两条路径并存** | ⚠️ 方向：评估统一到对象池（`USE_POOL` 开关已留，A/B 性能对照） |
...
2. **敌机生成路径统一**：评估池化统一（A/B 性能对照），对齐"所有敌机池化"事实。
3. **死代码清理**与幂等守卫（§7.3）。
