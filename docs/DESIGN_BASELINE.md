...
- **六型与加成**：标准型（基准）· 游隼（移速）· 重锤（攻击力）· 连弩（攻速）· 壁垒（防御）· 巨像（血量）。
  ...
  面板上显示的百分比由 `MachineTraitText.Percent` **按实际乘区反算**——改数值一处，面板文案自动跟随。
- **选择面＝机型轮盘**（2026-09-19 由列表改轮盘）：左缘圆盘排六型（全站菜单导航面统一为圆盘，
  ...
  巨像 装甲环带逐段点亮 / 标准型 校准环）；铭牌写机型名与加成 / 代价两行。
...
**面板与教学**：机型轮盘铭牌的能力项**并入既有「加成 / 代价」两行**（不加第三行——加行会挤掉板下的 note 与提示行），
为此板宽 460→560（板心不动、仍正对机体），使内容框装得下合并后的最长行；尺寸与对中算式单源在 core
`MachinePlateLayout`，判据 `MachinePlateLayoutTests`（内容框 ≥ 最长行 + 余量、板心对齐、不压轮盘）。
能力项**一律写自然量的秒数**（「冲刺冷却 3.4s」而不是「−15%」，避免加成行里出现负号）；每型另有一个 2–4 字性格标签
（基准 / 突进 / 重击 / 连射 / 坚壁 / 续航，键名由 core `MachineSpec.TagKey` 派生）。**教程不为机型分叉**：
教程跑在玩家选中的机型上，他学动作时顺带学到自己那台的手感；正确的位置是选择面（铭牌）。
