...
**定位**：开局前在标题屏选一种机体（`M` 键 / 底部「机型」入口）。六型 = **标准型**（无加成，沿用既有外观与数值）
+ 五特种型（各一项加成、各一套外观）。**不构成局外成长**：无解锁、无累积、不写任何进度——它是一次起飞配置选择，
...
**取值定稿**（结论：按表落地；依据＝`REFERENCES.md` §4.15；改 `data/balance.json machines.*` 一处即调）：

| 机型 | 加成项 | 值 | 依据 | 代价 |
| --- | --- | --- | --- | --- |
| 游隼 | 移动速度 | ×1.15（+15%） | 亚一层增幅；「速度是双刃剑」——快＝灵活但难走细缝 | 微调（Ctrl）与加速（Shift）的**绝对**速度同步抬升，细缝与贴身擦弹更难做 |
| 重锤 | 攻击力 | ×1.2（+20%） | 低于一层 `power_shot`（×1.25） | 伤害集中在单发：残血敌机上的溢出更多，等同损失 |
| 连弩 | 开火间隔 | ×0.85（+17.6%） | 低于一层 `rapid_fire`（×0.75）；射速另吃齐射计数与暴击次数的顺带收益 | 同屏弹体与枪口特效更密（可读性）；迷雾的间隔抖动按更短的基准抖 |
| 壁垒 | 受到伤害 | ×0.85（−15%） | 与一层 `armor` 同档，但**独立于加点**（裸机也硬） | 小额连续擦伤上的体感弱于大额；与护甲叠乘后数值离整数更远 |
| 巨像 | 血量上限 | ×1.2（+20 HP） | 低于一层 `extra_life`（+50 HP） | 回满耗时更长（被动回血与基地补给都是定额或一次性），重伤档停留更久 |
| 标准型 | 无 | ×1.0 | 保住既有玩家的默认体验：旧档归一到它 | 无 |

**为什么不是「加成 + 惩罚」**：该类型的惯例取舍是设计出来的（Cave 用「火力买机动」，见 §4.15 引文），
但本批按人类要求只给加成——取舍因此只来自「六选一」的机会成本。故量级一律压在**亚一层增幅**之下：
既让任一型都读得出、算得到，又不足以盖过本局的天赋成长，也不至于让某一型成为无脑选择。
