...
**开机链路**：`scenes/main.tscn` → 深空机库标题屏（`scenes/title.tscn`，形态见 §1.12）→ 新的一局 / **C 继续上次出击** / T 教程 / P 练习 / M 机型（六选一，见 §1.17）。
**本局存档**（2026-09-10 追加，口径见 §2.5）：回基地与选择「保存并退出」时落盘、死亡即删档、读档还原进度并从新一波开始。
**内容边界**：只解冻「记录 / 练习 / 表现」三类元层项（口径见 §1.16）；玩法系统扩展（新 Boss / 遭遇 / 天赋）冻结，重启须显式决策（见 ROADMAP）。**例外：机体于 2026-09-19 按人类指令解冻**，落地为「开局六选一」的初始机型（§1.17）。
...
- **Structure**：27 节点 = 4 大类 × 线（`csharp/core/Talent/TalentTree.cs` 是结构单源）。线序 = 前置链（下一节点需上一节点 ≥ Lv1）。节点上限 = `augments.<id>.max_stacks`（json 唯一权威；`extra_life` 10）。**节点规模与解锁口径只有 `max_stacks` 一条**——项目没有解锁条件机制（无 `unlock_*` 键）、没有权重池配置；**节点全部是强度节点**（无解锁语义节点：`phase_dash` 曾是唯一例外，已随冲刺开局可用退役，见 §1.19）。
...
- **母舰与基地**：召唤＝长按 H（`mothership.dock_charge_time` 3s，松手取消、不进冷却；被冷却 / 遭遇事件门控挡下时坞态灯给否认脉冲 + `UiDeny` 低音）→ 触发帧：链路锁定拍（落点冲击环 + 软闪，`CinematicFx.SummonTriggerBeat`）+ 穿梭门张开 + 母舰同帧穿出（`effects.mothership_summon.warp_in_time` 0.8s，门开到八成时舰首已破门）→ 到位：减速带 → 牵引进保护舱。**触发不锁输入**（无敌窗口起点与旧口径同为触发帧，只是不再连操作一起收走），锁输入起点＝牵引；触发到再可控 ≈2.8s（其中 0.8s 可操作）。驻留期火力平台 GATLING / MISSILE 交替；返航＝长按 B（`effects.home_charge_time`）→ 锁输入 → 停 spawner → 召回 → `starfield.Warp(18)` → 过场 → 基地 UI（树暂停）。基地 = `BaseConsole.cs` + `DawnStation.cs` 皮（左缘轮盘目录：机库 / 补给 / 契约 / 任务 + 「继续出击」叶；右区单面板切换），「继续出击」→ 轨道打击清场（Boss 保留）→ 入场动画。
...
- **冲刺 / 弹反 → `AbilitySocket` 充能槽**（同一构件两个实例）：充满即可用、用掉清空、冷却回充 → 环形冷却（一眼回答「能不能用」）。两态即读数：充能中＝环按进度填充 + 字形压暗、就绪＝点亮 + 满环 + 一次性外扩脉冲（原有的「未解锁」第三态随冲刺开局可用整条退役，见 §1.19）。**不用指针表盘**（本盘没有需要读速率的量）。
...
### 1.11 Tutorial
独立场景 `scenes/tutorial.tscn`，自管返回（不走 `BackNavigator`）；隔离本局态与存档，退出还原 `Engine.TimeScale = 1`。**七阶段**（实战与母舰停靠之间是弹反教学段），与正局对齐：阶段 1 强制标记靶、阶段 5 走生产停靠链。**阶段 2 不再白送冲刺**——冲刺开局即可用（§1.19），该段由「先给再教」变成「直接教」，文案补「冲刺期间免疫伤害」（冲刺的全部竞争价值在 0.25s 完全免伤，不写这一句会被读成「一个耗油的位移」）。
...
- **增幅 / 天赋轻量触点（阶段 6 基地段）**：返航开基地后，目标行点明增幅与天赋面板的键，实际打开一次增幅面板即过关；教程内自建同款面板（读生产增幅数据），零新经济链。**教程局内没有任何增幅**（不再白送一级 `phase_dash`），故该面板常态走既有空态分支（`TUT_AUGMENT_EMPTY`）——过关判据是「实际打开一次」，与列表里有几行无关。
...
- 栈：L3 退出确认 → L2 覆盖层（设置 / 基地 / 结算 / 过场）→ L1 本局（HUD ⇄ 暂停 + 增幅坞）。标题屏 `title.tscn` 为独立场景，自下而上四层：**程序化星空**（`Starfield`）+ **深空纵深层**（`TitleBackdrop`：远景行星 / 残骸带 / 尘埃近景 / 航道浮标，逐次进场随机取贴图与构图位）+ **远景实况战场**（`WarzoneRoot`：敌机编队 / 交火爆炸 / 曳光火网 / 大型舰与母舰巡航 / Boss 剪影 / 传感扫描）+ **悬挂展示与标题区**（玩家机自远处跃迁飞入，轮廓背光 / 尾焰怠速 / 铭牌卡），开场演出约 2.2s 不阻塞输入。**展示机由 core `MachineBag` 轮展**（口径见 §1.17）、**战场演出由 core `AmbientDirector` 抽取**（加权 + 逐类冷却 + 不连出同类），两处随机都不落盘、不影响任何一局；背景元素取位单源在 `TitleScreen.SkyZones`——让开左侧标题区与右侧悬挂展示位，压在文字或机体上不是「更丰富」，是更难读。
...
玩家可见内容只解冻「记录 / 练习 / 表现」三类元层项；玩法系统扩展（Boss / 遭遇 / 天赋）冻结（机体已按人类指令解冻，见 §1.17）。实现细节以代码为准。

- **本局记录（`user://best.json`）**：跨局保留的**局末结果读数**——存活时长 / Boss 击杀 / 最高难度档 / 本局目标是否达成（标题屏与结算页各一行）。边界：**不含战力、不含分数**（计分仍只是隐藏进度引擎），故不构成局外成长；与 `run.json` 分区，死亡删档不影响它，读取三态口径同本局存档；写入＝本局终结（死亡 / 放弃重开），仅在实际有改善且盘上记录可读时落盘；字段编解码单源 core `BestRecordCodec`（写读共用同一批键名）。**另存本局的机型 id**（`machine`，与 `run.json` / `settings.json` 同拼写、旧档缺键归一标准型）：它是**机型层唯一的回流数据**，**不上屏、不参与比较**——四项读数是逐项取更优的结果，给整行贴单一机型标签会读成「这一行是这型打出来的」。
...
**定位**：开局前在标题屏选一种机体（`M` 键 / 底部「机型」入口）。六型 = **标准型**（无加成、无代价，
沿用既有外观与数值）+ 五特种型（**各一项加成 + 各一项代价**、各一套外观）。**不构成局外成长**：无解锁、无累积、不写任何进度——它是一次起飞配置选择，
与设置里的难度档、开火方式同类，因此不动必死曲线与敌强曲线（`§1.4`）。

- **六型与加成**：标准型（基准）· 游隼（移速）· 重锤（攻击力）· 连弩（攻速）· 壁垒（防御）· 巨像（血量）。
  特种型一律「**一项加成 + 一项代价**」，代价与加成**异轴**（同轴相抵只剩中间值，读不出机型性格）。
  生效乘区单源在 `data/balance.json machines.<id>.*`（缺键回退 core `MachineRoster` 的名册默认值），
  标题屏悬挂铭牌上的百分比由 `MachineTraitText.Percent` **按实际乘区反算**——改数值一处，面板文案自动跟随；
  机型轮盘铭牌不印百分比，它画的是六条轴相对标准型的倍数（§1.19）。
- **选择面＝机型轮盘**（2026-09-19 由列表改轮盘）：左缘圆盘排六型（全站菜单导航面统一为圆盘，
  机型选择此前是唯一一页列表），右区展示被选中的那一架——换焦点时旧机向上右加速飞出、
  新机自远处跃迁飞入（光线场 + 拖影 + 制动冲击环），落位后播**该型的特性演出**
  （`MachineShowcase`：游隼 横移冲刺 / 重锤 重炮出膛 + 后坐 / 连弩 三连点射 / 壁垒 护盾环弹开来袭弹 /
  巨像 装甲环带逐段点亮 / 标准型 校准环）；铭牌写机型名 + **六维性格指纹图** + 一行性格文案（见 §1.19）。
  **浏览不写入**：转动轮盘只换焦点与展示，写机型只有一条路——回车 / A / 点机体（走生产单口
  `GameState.SetMachine`）；确认不关面板，铭牌标签随之在「现役机型 / 候选机型」间切换，Esc 退。
- **生效路径（三处消费点，别漏）**：机动 / 射速 / 攻击力在 `Player.ApplyMachineFactors`（启动落一次 +
  `MachineChanged` 时重落；**刻意不重跑 `LoadBalance`**，那条路会白送满无敌并回满燃料）；
  血上限在 `GameState.ApplyMachineHealth` 折进 `MaxHpBase`（故**吸血定额按「基础上限」同口径跟进**，
  不给重装型留隐形惩罚）；受到伤害乘在 `DamageMitigation.Resolve` 的护甲之后——与护甲增幅**叠乘且互不遮蔽**
  （护甲要加点才有，机型开局就带）。
- **一次只飞一型，本局不能换机**：偏好在 `settings.json machine`，本局机型在 `run.json machine`；
  「继续上次出击」以存档为准并同步回偏好。**旧档（两处都没有这个键）一律归一标准型**——
  **数值乘区与贴图**与加机型之前逐位一致（能力轴四条全基准 1.0；**冲刺的可用性已移到基准层、
  与机型无关**，见 §1.19，故「逐位一致」不含它）。
- **外观**：六型共用同一套贴图锚点（254×254、机头 (127,16)、翼尖 (12/242,206)、喷口 (108/146,230) 等；
  单源在 `generate_player_sprite.py` 头部注释与 core `PlayerHullLayout`），靠剪影 / 板划分 / 暖族内色调区分；
  素材一律走确定性生成器（重跑 `regenerate_all.sh` 后 `git diff` 为空）。
  **五型各一件「看剪影就认得出」的专属特征**（2026-09-19 补，翼型同时拉开：薄刃后掠锯齿 / 短展厚弦截角 /
  折线前缘 / 截角方翼尖 / 深弦宽翼）：游隼 翼尖外挂短舱 + 翼根长边条、重锤 肩部双联重炮 + 翼下六管火箭巢 +
  机首撞角犁、连弩 机首并列双炮管（冷却套）+ 翼根弹鼓与弹链、壁垒 前缘五段叠层装甲带 + 翼面盾形甲片 +
  机背大盾板、巨像 翼下八角外挂装甲舱 + 机背五段环带。特征一律做到 ≥8 贴图像素并自带棱线与板缝——
  主要观看面是机型面板 1.1× 与标题屏悬挂 2×，只能凑近看的碎花在面板上读不出。
  **标题屏尾焰与喷口辉光的取位**走 core `PlayerHullLayout.EngineLeft/Right`（＝骨架喷口 108/146, 230 ×
  悬挂缩放），与机体贴图同一坐标系；写死坐标时尾焰会从机身中段冒出来（反转链见 ROADMAP 决策条目）。
- **标题屏展示机＝机库轮展**：每次进标题屏由 core `MachineBag` 从六型里抽一架悬挂（无放回洗牌，
  一袋之内六型各一次、跨袋也不连出同型），铭牌写机型名与该型加成，标签随「是否等于此刻生效的
  `GameState.MachineId`」在「现役机型 / 机库展示」间切换；机型面板选定后展示立刻换成刚选的那一型
  （原有反馈保留）。**节拍**：自进场起每 **3.4s** 换下一架（2026-09-19 补，原为「每次进标题屏只换一架」），
  换机走「先隐后显 + 轻微缩放弹」的过渡（不加色闪光）；**面板打开期间不轮换、换机与关面板都把节拍重起**
  ——玩家正在逐型比对或刚做完选择时，展示机不能在他眼皮底下被换走。节拍是常量
  `TitleScreen.ShowcaseInterval`（展台节拍、非玩法数值，故不进 `balance.json`）。
  **为什么不逐次独立掷签**：六型里连出同型的概率是 1/6，玩家一晚上回标题屏十几次
  必然撞见，那一瞬间读到的不是「随机」而是「没换」。轮展状态是进程内的静态袋，不落盘、不影响本局。
- **不做的**：机型专属武器形态（Cave 式 A/B/C 差异）、机型解锁与升级、机型影响得分。
  （机型性格的三层：①表现层 / ②手感层见 §1.18，**③玩法层能力强度差异见 §1.19**。）

**取值定稿**（结论：按表落地；依据＝`REFERENCES.md` §4.15 / §4.16；改 `data/balance.json machines.*` 一处即调）：

| 机型 | 加成 | 代价（减益） | 依据 | 手感备注 |
| --- | --- | --- | --- | --- |
| 游隼 | 移动速度 ×1.15（+15%） | **攻击力 ×0.9（−10%）** | 快而火力单薄；「速度是双刃剑」（§4.15） | 微调与加速的**绝对**速度同步抬升，细缝更难做 |
| 重锤 | 攻击力 ×1.2（+20%） | **移动速度 ×0.9（−10%）** | Cave 式的「火力买机动」反向（§4.15 引文）；重炮平台 | 单发伤害集中，残血敌机上溢出更多 |
| 连弩 | 开火间隔 ×0.85（+17.6%） | **受到伤害 ×1.12（−10.7% 等效耐久）** | 高射速换装甲（弹链外露）；§4.16 | 同屏弹体更密；齐射计数与暴击次数推进更快 |
| 壁垒 | 受到伤害 ×0.85（+17.6% 等效耐久） | **开火间隔 ×1.12（−10.7% 射速）** | 装甲换火力；§4.16 | 与护甲增幅叠乘后数值离整数更远 |
| 巨像 | 血量上限 ×1.2（+20 HP） | **移动速度 ×0.88（−12%）** | 最重的机体（厚＝慢）；§4.15 | 回满耗时更长（回血与补给是定额或一次性） |
| 标准型 | 无 | 无 | 保住既有玩家的默认体验：旧档归一到它；也是「不承担任何取舍」的基准 | 无 |

**加成与代价的三条规则**（依据见 §4.16）：

1. **量级压在亚一层增幅之下**：既让任一型都读得出、算得到，又不足以盖过本局的天赋成长。
2. **代价无条件、与加成异轴**：条件性代价会被玩家折扣掉（§4.16 引文），同轴相抵则读不出机型性格。
3. **代价幅度约为加成的 0.5–0.7 倍**：损失在体感上约为等量收益的两倍，1:1 的数值交换体感是净亏；
   而象征性小代价又抵不住收益。判据钉在 `MachineRosterTests.PenaltyMagnitudeStaysBelowTheBonusButIsNotToken`（比值带 [0.4, 0.9]）。

**取舍的读法**：游隼↔重锤是「速度 ↔ 火力」的两端，连弩↔壁垒是「射速 ↔ 装甲」的两端，巨像拿机动换血量；
标准型不承担任何取舍、也没有任何加成——它是基准点，不是「更强的那一型」。

### 1.18 机型手感档案（六型表现 / 手感差异）

**定位**：把机型轮盘特性演出的动作语汇搬进游戏内（ROADMAP 2026-09-19 设计稿实验的落地）——
机型差异从「五条乘区 + 外观」扩到**飞行中的机体读得出性格**。三层里已落 ①表现层与 ②手感层：
**判定几何（碰撞圆 / 擦弹环 / 弹道）、玩法数值（五条乘区 / 敌弹 / 刷怪）逐位不动**；
③玩法层（开局能力 / 能力强度差异）见 §1.19。对照基准是标准型（档案全基准：既有玩家的旧画面逐位一致），差异小到读不出＝白做、
大到能换胜负＝隐形收益，两层都算失败。

**落地形态**：core `MachineFeel`（22 字段纯表，零 Godot 依赖）＋ 名册默认表 `MachineFeelTable`
（域收口 0.2–3.0、未知 id 回退基准）＋ `balance.json machines.<id>.feel.*`（**只列差异键**，未列即
core 默认；标准型不列分区）。引擎层只消费：表现参数乘区收口在 `PlayerVisuals.ApplyFeel`
（幂等全量重算，振幅键的代码默认收成 const 防叠乘）；命中反馈（顿帧时长 / 直击震动 / 弹着火花）
在 `Bullet` 直击路径与 `GameFeelService`；损伤帧阈值与残影间隔在 `Player.ApplyMachineFeel`。
生效节奏与乘区相同：启动一次 + 换机重跑。判据钉在 `MachineFeelTests`（域 / 回退 /
六型两两不同 / 设计稿条目映射 / balance 对账与键白名单）。

**取值定稿**（结论：按表落地；依据＝动作语汇与轮盘演出同源 + 量级锚定 §2.13/§2.16/§2.17/§2.18 既有键，
不越过同量级线；改 `data/balance.json machines.<id>.feel.*` 一处即调）：

| 档案字段（键） | 游隼 | 重锤 | 连弩 | 壁垒 | 巨像 | 对应设计稿条目 |
| --- | --- | --- | --- | --- | --- | --- |
| `dash_pop_scale_mult` 冲刺弹跳幅度 | 1.5 | — | — | — | — | 游隼② 弹跳更大 |
| `sway_mult` 转向跟随幅度 | 1.4 | — | — | 0.55 | — | 游隼② 更「飘」/ 壁垒② 更「钝」 |
| `bank_mult` 横移侧倾幅度 | — | — | — | 0.55 | — | 壁垒② 机动更钝 |
| `vortex_threshold_mult` 涡流门槛 | 0.6 | — | — | — | — | 游隼② 涡流更早 |
| `recoil_px_mult` 开火后坐位移 | — | 1.6 | — | — | — | 重锤② 后坐更大 |
| `hit_impact_mult` 命中顿帧时长 | — | 1.35 | — | — | — | 重锤② 顿帧更强 |
| `direct_shake` 直击震动（px） | — | 5 | — | — | — | 重锤② 命中震动（基准 0） |
| `lag_px_mult` 滞后漂移（加速更沉） | — | 1.5 | — | — | 1.45 | 重锤 / 巨像② 加减速更沉 |
| `fire_light_mult` 开火机身光 | — | — | 1.45 | — | — | 连弩② 机身光更密 |
| `fire_jitter_px` 连射机身微抖（px） | — | — | 1.2 | — | — | 连弩② 连射微抖（基准 0） |
| `hit_squash_mult` 受击压缩幅度 | — | — | — | 0.5 | 0.7 | 壁垒 / 巨像② 回弹更小更「扛」 |
| `parry_glow_mult` 弹反环亮度 | — | — | — | 1.35 | — | 壁垒② 弹反环更亮 |
| `bob_px_mult` 悬停浮动幅度 | — | — | — | — | 0.5 | 巨像② 悬停更稳 |
| `afterimage_life_mult` 残影寿命 | 1.6 | — | — | — | — | 游隼① 残影更长 |
| `afterimage_rate_mult` 残影密度 | 1.5 | — | — | — | — | 游隼① 残影更密 |
| `dash_speedline` 冲刺速度线强度 | 0.85 | — | — | — | — | 游隼① 速度线（基准 0＝旧体验） |
| `muzzle_glow_mult` 炮口辉光尺寸 | — | 1.45 | 1.15 | — | — | 重锤① 炮口冲击更大 / 连弩① 枪口闪 |
| `hit_spark_mult` 弹着火花尺度 | — | 1.4 | — | — | — | 重锤① 弹着更重 |
| `tracer_len_px` 弹道曳光（px） | — | — | 22 | — | — | 连弩① 曳光成串（基准 0） |
| `damage_frame_mult` 损伤帧阈值倍率 | — | — | — | 0.7 | 1.35 | 壁垒① 损伤帧更晚 / 巨像① 更早 |
| `smoke_min_level` 损伤烟起始档 | — | — | — | — | 1 | 巨像① 损伤烟更早（轻伤档） |
| `deflect_read` 受击被弹开读数 | — | — | — | 1 | — | 壁垒① 环闪＋火花外溅（基准 0） |

（「—」＝不列键取基准 1.0 / 0。六型两两不同由单测钉住；上表未列的巨像「损伤分段（甲片逐块变暗）」
以「损伤帧更早 + 损伤烟提前到轻伤档」等效表达——逐块甲片分层是贴图工程，与本轮实验同收口条件的
轻量读数先行。）

**准入判据自查**（§2.12 六条同口径）：新增动效全部事件驱动、局部元素（环闪屏占 <5%、速度线走
细线运动层峰值 alpha 口径）；曳光 ZIndex 垫在弹体后、与弹体同生共死（拖尾不比判定活得久）；
连射微抖与环闪相位只取模拟时间（确定性双正弦，无随机源）；直击震动走 `GameState.Shake`
（受「屏幕震动强度」折算）、环闪受「减少闪光」折半、微抖与档位乘区受「动效强度」缩放、
冲刺速度线与战况响应同一动效强度判定口径；残影寿命 / 密度是既有功能的参数化（改时长不改振幅），
不受动效强度缩放。

### 1.19 机型能力轴（玩法层差异）

**定位**：机型差异的第三层——**开局能力的强度差异**（①表现层 / ②手感层见 §1.18，数值乘区见 §1.17）。
与乘区同层：一次起飞配置，**不构成局外成长**（无解锁、无累积、不写进度）。
**前提**：冲刺**开局对六型一律可用**（不再是 `phase_dash` 层 1 的解锁奖励，见 §1.5 的解锁口径）——
机型差异落**强度**不落「有无」（「有无」是二元差、且它的价在账本最小刻度之下，收不出代价）。

**四条轴**（单源 `data/balance.json machines.<id>.kit.*`，只列差异键；缺键回退 core `MachineKitTable` 名册默认、非有限回 1.0）：

| 轴 | 键 | 乘在哪（基准值） | 面板自然量 | 基准 | 感知下限 |
| --- | --- | --- | --- | --- | --- |
| 弹反激活窗 | `parry_window_mult` | `player.parry.active_time`（0.5s） | 窗口秒数 | 0.50s | 0.15s |
| 弹反循环 | `parry_cooldown_mult` | `player.parry.cooldown`（3.0s；面板读**循环**＝0.8＋冷却） | 循环秒数 | 3.80s | 0.30s |
| 冲刺冷却 | `dash_cooldown_mult` | `player.dash.cooldown`（4.0s） | 冷却秒数 | 4.00s | 0.40s |
| 加速耗油 | `fuel_drain_mult` | `player.fuel.drain`（35/s） | 满箱可持续加速时长（100÷耗油） | 2.86s | 0.30s |

**域**：弹反窗 `[0.4, 1.5]`、其余 `[0.4, 2.0]`；**越界取边界值**（回 1.0 会把 >1.0 的代价静默抹平）、
NaN/Inf 才回 1.0。弹反窗上限 1.5 而不是 2.0：`ParryTimeline` 把 `ActiveTime` 钳到流程时长 0.8s，再高是死区。

**取值定稿**（改 `data/balance.json machines.<id>.kit.*` 一处即调；面板百分比 / 秒数一律由实际乘区反算）：

| 机型 | 能力强化 | 值（面板读数） | 能力代价 | 值（面板读数） | RU 比 |
| --- | --- | --- | --- | --- | --- |
| 游隼 | 冲刺冷却 | ×0.85（4.0→**3.4s**） | 弹反循环 | ×1.1（3.8→**4.1s**） | 0.67 |
| 壁垒 | 弹反激活窗 | ×1.4（0.50→**0.70s**） | 冲刺冷却 | ×1.1（4.0→**4.4s**） | 0.75 |
| 连弩 | 弹反循环 | ×0.85（3.8→**3.35s**） | 加速耗油 | ×1.15（续航 2.86→**2.48s**） | 0.83 |
| 重锤 | 加速耗油 | ×0.87（续航 2.86→**3.28s**） | 冲刺冷却 | ×1.1（4.0→**4.4s**） | 0.70 |
| 巨像 | 加速耗油 | ×0.82（续航 2.86→**3.48s**） | 弹反循环 | ×1.15（3.8→**4.25s**） | 0.72 |
| 标准型 | 四条轴全基准（1.0）——它是面板所有读数的参照物 | — | — | — | — |

**三条规则**（沿用 §1.17，`MachineKitTests` 钉住）：① 量级压在**亚一层增幅**之下（冲刺冷却 > `phase_dash`
一层 0.8、弹反冷却 > `deflector` 一层 0.78、耗油 > `efficient_boost` 一层 0.75、弹反窗 ≤ 1.40）；
② 代价无条件、与强化**异轴**；③ 两侧各自的**可读单位**（RU＝|自然量变化| ÷ 该轴感知下限）≥ 1.00，比值 ∈ [0.4, 0.9]。
**三条结构约束**（跨机型）：**K1** 强化轴与代价轴必须**分属不同能力**（弹反窗 / 弹反循环同属「弹反」，两者相抵会把账本读成中性）；
**K2** 弹反窗**不得作为任何机型的代价轴**（一次失败的弹反同样烧掉整个循环，窄窗单价极高且无处补偿；人类指令要的是「延长弹反时间」）；
**K3** 五型加成行的（轴, 值）对**两两不同**（四轴 × 五型是鸽笼，重复被刻意压在读数最弱的加速轴上，且两值必须不同）。

**读数分级（诚实口径，不许含糊成「四条轴都同样有效」）**：弹反窗与冲刺冷却是**体感轴**（不看面板就能感到）；
弹反循环是**面板轴**；加速耗油是**准体感轴**（按住加速时油表下降速率可感）。故连弩 / 重锤 / 巨像的「鲜明特色」
主要由数值层与手感层承担，能力层交付的是**账本意义的差异**。

**面板与教学**：机型轮盘铭牌写**六维性格指纹图** + 机型名 + 一行性格文案
（`MACHINE_BLURB_*`，图是纯视觉编码，这一行是它的文本等价物）。指纹图**不再逐项印数字**——
六型差异压在亚一层增幅之下，按绝对刻度画六个多边形几乎重合，故刻度口径是**相对标准型的倍数**：
中心 ＝ 标准型基准、基准环在一半半径处（凸起＝强于基准、凹陷＝弱于基准），刻度带 **[0.60, 1.40]**
（当前全体取值域 0.870–1.400，取整做成对称带；将来越界钳到边界并重定此表）。
算式与几何单源在 core `MachineProfileTable`（`csharp/core/Machines/MachineProfile.cs`），
逐格取值由 `MachineProfileTests` 钉住：

| # | 轴（键 `MACHINE_PROFILE_*`） | 算式（全部为「相对标准型的倍数」，1.0 ＝ 基准，越大越有利） |
| --- | --- | --- |
| 1 | 火力 `Fire` | 攻击力乘区 ÷ 开火间隔乘区 |
| 2 | 机动 `Mobility` | 移动速度乘区 |
| 3 | 装甲 `Armor` | 血量上限乘区 ÷ 受到伤害乘区 |
| 4 | 弹反 `Parry` | （弹反窗乘区 × 3.8）÷（0.8 ＋ 3.0 × 弹反冷却乘区）＝盾的有效占空相对基准 |
| 5 | 突进 `Dash` | 1 ÷ 冲刺冷却乘区 |
| 6 | 续航 `Endurance` | 1 ÷ 加速耗油乘区 |

每型另有 2–4 字性格标签（基准 / 突进 / 重击 / 连射 / 坚壁 / 续航，键名由 core `MachineSpec.TagKey` 派生）。
**教程不为机型分叉**：教程跑在玩家选中的机型上，他学动作时顺带学到自己那台的手感；正确的位置是选择面（铭牌）。

**铭牌版式定稿**（2026-09-19；结论：机体上移、板加高到 560×397，值如下表）——纵向关系与判据
单源在 core `MachinePlateLayout`，`MachinePlateLayoutTests` 钉住（板心仍正对机体停驻位、
板顶不压机体实心像素、机体不压页题、板底不压 note 与提示行、图表盒 + 文本区装得进板内、
文本区装得下最长一行 + 40px）：

| 量 | 值 | 依据 | 代价 |
| --- | --- | --- | --- |
| 机体停驻位 | (1160, **313**)，原 y=420 | 板加高只能吃上方空间（板下封在 note / 提示行里）；机体实心下缘 557 + 15 余量 ＝ 板顶 572 | 机体整体上移 107px；页题随之压到顶带 y=16 |
| 铭牌 | 560×**397** @ (880, **572**)，原 560×168 @ (880, 686) | 两行数字换成指纹图 + 一行文案；板高 ＝ 内容 26/10 内缩 + 文本区 108 + 图盒 253 | 板下 note 与提示行各下移（note 981、提示 1020，离屏 33px） |
| 指纹图 | 盒 253 正方形，半径 **96.5**，轴标签留白 30 | 图是这一页的主角：板加高的空间全部给图；半径 ＝（253 − 2×30）÷2 | 板宽 560 里的水平余量用不满（图是正方形，受高度约束） |
| 文本区 | 三行 108px（27/45/30 + 行距 2×3） | 行高按 `NotoSansSC` 实测（16px→24 / 28px→42 / 18px→27）加 3px 余量 | — |

（口径同 §1.19 的其余取值：改 `MachinePlateLayout` 一处即调，判据与观感一起走。）


**不纳入的轴**（各自有更硬的理由）：母舰蓄力 / 离舰冷却（与 `mothership_recall` 同乘区、60s 量级过大）、
弹反反射伤害（与 `deflector.reflect_mult` 同乘区，且属伤害轴）、燃料容量（与冲刺成本按比例耦合）、
冲刺距离 / 时长（判定几何，能换胜负）、冲刺撞伤 / 弹反多充能 / 弹反无敌（玩法系统扩展）。

**难度与对账**：机型层的**综合容错**（输出 × 有效生命 ÷ (1 − 冲刺免伤占空比@满投资)）跨度不得大于现状 **1.325×**
（防回归，`balance_analysis.py` 的「机型能力层」节钉住）；冲刺满投资免伤占空比 16.13%，物理上限 16.67%
（燃料地板 `T_min = dash.time + fuel.max × fuel.ratio ÷ fuel.regen = 1.500s`）。
...
### 2.18 机体形态层
主题是**机体的「轮廓」随状态改变**。§2.13 / §2.16 / §2.17 已交付的表现全部是在同一副机形上加光、加色、加位移——玩家看得见「更亮 / 更热 / 更抖」，却读不出「现在在干什么」。本层让外形本身成为读数：**待机会收枪、开火会伸枪、急转会划出蒸气**。依据见 `REFERENCES.md` §4.14（剪影先于配色、可收放炮塔与可变几何的真实工程惯例、高 G 翼尖涡流、金属红热温色表）。**三判据**（沿用 §2.12）：① 全程只写贴图节点及其子节点的变换 / 颜色 / 显隐，机体根节点、碰撞圆、擦弹环、弹道判定逐位不动；② 一切读数**受 `fx_intensity` 缩放**，0 ＝ 整层不出现（本层含不透明几何，故是整层隐藏而非 alpha 归零）；③ 相位只取累计模拟时间，零随机源，无头固定步长可重复。

**四构成**：

① **收放式机炮**：机身两侧肩部各一具荚舱，炮管在开火后 0.12s 内伸出、停火 0.55s 后收回（伸出快、收回慢——开火要跟手，收枪可以缓）；**冲刺期间强制收拢**（气动收枪，与「可变后掠翼随速度改变几何」同一语汇）。炮管多边形按全长预分配、伸缩只改 `Scale.Y`，零逐帧分配。
② **炮管热量**：每次开火升温、停火按 τ1.1s 指数降温；炮管着色走**暗钢→琥珀→白热**两段插值，炮口辉光随热量点亮，高热时枪管微抖。**只做读数、不做惩罚**——热量不进任何玩法结算（引入过热惩罚＝改玩法数值，须先按 §2 定稿）。
③ **散热排气**：热度越过 0.25 即从**机背脊线**两侧放热（排气口随热度变大而非只变亮——实测加亮到饱和也只有 3 世界像素、读不出来）；**弹反成功把热量清零并一次性猛喷**——盾把吸收的能量倒掉，把散热口接到唯一的防御动作上。
④ **翼尖涡流**：横向加速度占比越过 0.35 才在翼尖出现，强度同时给长度与亮度（「拉长」比「变亮」更像真蒸气）；直线巡航不出现——涡流是机动读数而非常驻装饰。

**数值与单源**：`effects.motion.player_gun_{deploy_rate,retract_rate,hold_time,heat_per_shot,heat_tau,vent_threshold,vent_alpha}` 与 `player_vortex_{alpha,threshold}` 九键；算式（保持窗、升温钳制、指数降温、排气与涡流的阈值斜坡、两段温色混合）单源 core `Visual/HullRig.cs` 并配单测；挂点几何单源 core `Visual/PlayerHullLayout.cs`。**推进走渲染帧**（`PlayerVisuals.UpdateHullRig`，由 `Player._Process` 驱动）而非物理帧：冲刺与入场期物理帧早退，而「收枪加速」这个读数恰恰只在冲刺期出现，放物理帧会被冻住。

**取值定稿**（依据＝§4.14 惯例 + 量级锚定 §2.13/§2.16/§2.17 键）：

| 项 | 值 | 依据 | 代价 |
| --- | --- | --- | --- |
| 保持窗 | 停火 0.55s 后收 | 短于它会在连点之间反复收放（像故障）；长于 1s 会让「停火」这个读数消失 | 停火后 0.55s 内机体仍是「张开的」 |
| 伸缩速率 | 伸出 16s⁻¹ / 收回 5.5s⁻¹ | 伸出 ~0.12s 跟上开火（射击手感要跟手），收回 ~0.4s 读作「收枪」而非「枪掉了」 | 极限连点下炮管一直在动（读数即「在打」） |
| 炮管长度 | 20 贴图像素（≈5 世界像素） | 量级锚定 §2.17 RCS 喷口（3.4px）与速度伸缩（±8%≈5px）：形态变化要压过姿态噪声才读得出 | 伸出态机身轮廓比判定长 5px（判定不动） |
| 每发升温 / 降温 | +0.14 / τ1.1s | 0.15s 射击间隔下约 1.1s 到白热、停火 ~2s 退到琥珀——读数跟得上「打了多久」 | 连发 1s 后炮管常驻白热（读数即「正在压制」） |
| 排气阈值 | 热度 0.25（约 2 发） | 低于此值「刚打完」与「没打过」看起来一样；高于它连点两下就常冒热气 | 轻点两枪也会放一次热 |
| 排气口尺寸 | 直径 11→24 贴图像素（随热度） | 尺寸而非亮度才是这里的读数：加亮到饱和实测仍只有 3 世界像素，肉眼看不出（同一位置的像素统计：冷却相位比静息多 55% 的亮像素） | 满热时脊线上两枚 6px 暖橙点 |
| 涡流阈值 | 横向加速度占比 0.35 | 低于它小幅修正也出涡流（常态机身挂着两条尾巴）；高于它要明显打舵才出现 | 只有明确横移才读得到机动 |
| 涡流全长 | 40 贴图像素（≈10 世界像素） | 实测 30（7.8px）时窗口化过目读不出「划出蒸气」；40 约机体长的六分之一 | 急转时翼尖外扩 10px 的冷白拖尾（无判定） |
