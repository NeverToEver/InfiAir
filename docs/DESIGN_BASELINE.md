...
- **不做的**：机型专属武器形态（Cave 式 A/B/C 差异）、机型解锁与升级、机型影响得分。
  （机型性格的 ①表现层 / ②手感层已落为 §1.18 手感档案；**玩法层**差异已获人类指令解冻（2026-09-19），
  设计稿与取值见 ROADMAP 工作计划 P1。）
...
③玩法层（开局能力 / 能力强度差异）已获人类指令解冻（2026-09-19），设计稿与取值见
ROADMAP 工作计划 P1。对照基准是标准型（档案全基准：既有玩家的旧画面逐位一致），差异小到读不出＝白做、
大到能换胜负＝隐形收益，两层都算失败。
