...
### 批次 1 — core 回归面补齐（纯逻辑单测，先红后绿）

摸底结论：core 13 个模块 9 个有单测（共 80 Fact），4 个零覆盖；它们写坏的表现（玩家看到
格式串、轮盘选不中条目、天赋前置漏判）现有门禁全部跑不到，且均属 §5 第一层「纯函数秒级可测」：

- **GdFormat**：自述全库唯一支持 `%.Nf` 的 GDScript `%` 格式化实现，含越界 `?` 兜底，玩家可见文案直接吃它的输出。收口＝全部格式符与兜底分支有用例钉住（`check_unit_tests.sh` 自动覆盖）。
- **RadialWheelModel**：core 最大文件（233 行）的轮盘导航状态机，标题 / 设置 / 暂停三处共用。收口＝Drill / Back / 焦点序 / 角度命中的状态迁移先红后绿。
- **TalentTree**：静态结构与前置链 / 互斥 / 路线查询。收口＝结构不变式入单测（前置链无环，互斥对与路线绑定的节点真实存在）。
- **TalentFanLayout**：扇形布局几何。收口＝输出的确定性入单测（每节点唯一坐标、落在 1080p 设计域、同输入同输出）。
