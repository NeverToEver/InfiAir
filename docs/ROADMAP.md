...
- **设置页窗口化过目（2026-09-11 规范化随行项）**：五页内容高度与左缘轮盘的排布只有真机可判——无头冒烟只保证「开页不崩」，不覆盖布局。需窗口化逐页过目：轮盘五项槽距、控制页滚动位置、音量滑杆手感、确认弹窗焦点、垂直同步/帧率读出是否随设置变化刷新。
- **轰炸编队深化后的人工过目（2026-09-11）**：无头冒烟只证明「不崩」，落点圈/倒计时弧的可读性、压坡姿态、反射弹的寻敌手感、`bomb_edge_falloff` 的实际容错宽度都只有实机可判。重点看：落点圈与 120px 实际伤害边界是否对齐、引信弧是否够醒目、弹反一枚后能否稳定命中编队（寻敌加速度 1600px/s² 的取值需实机手感定稿）、拦截奖与全歼奖的提示是否会被同伴台词顶掉。
