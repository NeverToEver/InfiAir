...
- **[低] 测试/探测开关仍在生产路径**：`--event-probe` / `--settings-probe` / `--startup-time` 读命令行并改启动流程，违反 AGENTS §3「测试设施不进生产路径」。收口方向：探针宿主场景 + 事件查询 API，把开关与驱动移出 `Main`；下次触摸 `Main._Ready` 启动分支时一并做。
...
- **2026-09-12 工程纪律重写**：门禁只判静默错误——退役两个文风门禁（散文与术语改约定 + review；黑名单式文风判定把注意力从正确性引向措辞），行为改动必须有回归面（引擎耦合行为走 `--fixed-fps` 无头固定步长，可下沉的判定下沉 `csharp/core/` 单测），测试与探测设施不进生产路径，数值/手感/产品范围归人。顺带落地：无头冒烟扩为四趟（编队 + 精英炮塔，均断言全周期完成标记），弹反运动学下沉 core 并由单测钉住「咬得住编队」。为什么：上一批「两轮人工过目全绿、弹反机制整体不推进」证明原判定链（文风门禁 + 人工过目）判不出行为错误。口径见 AGENTS.md。
