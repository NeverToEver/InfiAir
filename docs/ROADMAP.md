...
- **[低] 天赋扇形中线第 4 级节点越出面板顶缘（2026-09-12 单测发现）**：奇数支线的中线竖直向上，`offense.firecontrol` 的 `salvo` 与 `special.logistics` 的 `combo_guard` 卡片中心在面板局部 Y≈-51.4（卡 128px，大半在扇形区外）。Control 默认不裁剪子节点，卡片仍会绘制——是否与被盖住的上缘 UI 抢位需窗口化过目。为什么不现在做：`RootGap`/`RadiusStep` 是玩家可感布局参数（归人，§3），且视觉改动须窗口化人工过目（§4）。收口条件＝窗口化过目认可现状，或人类定新布局参数后更新 `TalentFanLayoutTests.RealTree_AllNodesStayInsideDesignDomain` 的例外钉值；触发时机＝下次窗口化过目天赋面板。护栏＝该单测已把两个越界节点钉在精确值，偏离即红。
