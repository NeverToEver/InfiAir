...
- **轰炸编队实机手感定稿（2026-09-12 首轮自动化过目后的剩余项）**：首轮已修复帧率档文案键直出、超宽行撑破设置面板、全歼奖静默无提示，并把反射寻敌加速度迁入数值单源（`bomb_reflect_turn_accel`）。剩余需真人窗口化实机定夺：① 炸弹爆炸视觉（`explosion_visual_scale` ×1.6）远大于 120px 伤害圈，多弹混爆时落点圈/引信弧被淹没——缩小爆炸视觉或提亮圈需二选一；② 弹反一枚后能否稳定命中编队（调 `bomb_reflect_turn_accel` 即可）；③ 压坡姿态动态观感与 `bomb_edge_falloff` 的实际容错宽度。
