...
- **轰炸编队实机手感定稿（2026-09-12 二轮过目后的剩余项）**：一二轮已修复帧率档文案键直出、超宽行撑破设置面板、全歼奖静默无提示、反射寻敌加速度入数值单源；二轮电影模式逐帧过目关闭了原 ①③——「爆炸淹没落点圈」的真正根因是落点圈/引信弧渲染 bug（Line2D 线宽随节点 Scale 同相放大，「单位圆 ×Scale」把环带放大成盖满圆心的实心巨盘），修复后圈/弧独立可读、爆炸（逐帧+日志证实 12 次全部 scale=1.1）不再淹没任何信息；压坡平滑生效、`bomb_edge_falloff` 蹭边 35% 衰减合理，均不改参数。同轮修复编队状态机巨帧连跳（开局卡顿帧实测 >1.4s，编队闪现投完弹即离场）并让 event-probe 真正覆盖状态机（原先被过场树暂停冻结，无头冒烟只测到 Start 不崩；现 probe 直进开局并锁 60 帧，冒烟 400→700 帧，并加「全周期完成」标记断言——帧数只是上限，卡在入场段同样是「零错误退出」）。本轮代码收尾另发现反射弹的推进被误关（`Reflect()` 里的 `SetProcess(false)` 把弹体冻在弹反点，寻敌转向与出界回收全走不到），已恢复推进，并按探针实测把弹反初速补进数值单源（`bomb_reflect_speed_mult`——原样奉还的初速只比编队横穿快一成，追尾永远差一截，真正的瓶颈是速度余量而不是转向加速度）。剩余真人项：**实机弹反手感确认**——探针三连跑均咬住编队（≈1.4s 命中、命即击毁），需真人确认这个回报力度是否过头；与 Cinematic stage 4、15 分钟真机手感两项既有真人项一起留待发布前。
