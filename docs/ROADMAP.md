...
- **[低] TalentPanel 接入 RadialMenuLayer 基类**：dim/holder/入场编排三件套与 Settings/BaseConsole/GameOver 重复（自管骨架平行实现）；2026-09-09 风格审查批次评估后暂缓——编排迁移属视觉结构改动，无头门禁无法验证效果等价性，需与窗口化人工过目配合执行（WheelRest 常量已先行单源化）。
