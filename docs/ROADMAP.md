...
- **轰炸编队实机手感定稿（2026-09-12 二轮过目后的剩余项）**：一二轮已修复帧率档文案键直出、超宽行撑破设置面板、全歼奖静默无提示、反射寻敌加速度入数值单源；二轮电影模式逐帧过目关闭了原 ①③——「爆炸淹没落点圈」的真正根因是落点圈/引信弧渲染 bug（Line2D 线宽随节点 Scale 同相放大，「单位圆 ×Scale」把环带放大成盖满圆心的实心巨盘），修复后圈/弧独立可读、爆炸（逐帧+日志证实 12 次全部 scale=1.1）不再淹没任何信息；压坡平滑生效、`bomb_edge_falloff` 蹭边 35% 衰减合理，均不改参数。同轮修复编队状态机巨帧连跳（开局卡顿帧实测 >1.4s，编队闪现投完弹即离场）并让 event-probe 真正覆盖状态机（原先被过场树暂停冻结，无头冒烟只测到 Start 不崩；现 probe 直进开局并锁 60 帧，冒烟 400→700 帧）。剩余真人项：**实机弹反一枚验证命中编队**——几何推断参数健康（寻敌转弯半径 ≈88px << 弹反距离，1600 加速度无需调），只差一次实证；与 Cinematic stage 4、15 分钟真机手感两项既有真人项一起留待发布前。
