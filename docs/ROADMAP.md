...
- 平衡审查与落地（2026-09-14）：`docs/BALANCE_REVIEW.md` 是快照于改动前的一次性**难度曲线/平衡机制/奖励机制/手感观感横向审查**（现状取证 + 行业惯例比对 + 分阶段路线），保留为决策依据与行业证据存档。按其症结排序的批次已全部落地并经人类实机核验：打击感层（命中顿帧 + 震动 trauma + 时间缩放合成器）、Boss 狂暴回血修复、难度映射单源化（core `DifficultyScaling`）与曲线重校（Boss 斜率独立、速度硬顶、精英随难度增长、开火地板、时间项软上限）、奖励随难度增长、缓存正向回补、本局达成目标和命名难度档位、受击喘息收窄、Boss 攻击密度接 D。**设计口径与定稿取值单源在 DESIGN_BASELINE**（§1.3 / §1.4 / §1.4.1 / §1.4.2 / §1.4.3 / §1.5 / §2.9），本文件不再复述。
...
- **2026-09-14 平衡审查落地批次**：按 `docs/BALANCE_REVIEW.md` 的症结排序分阶段落地——打击感层（顿帧 + 震动 trauma + 时间缩放合成器）、Boss 狂暴回血修复、难度映射单源化（core `DifficultyScaling`）并重校曲线（Boss 斜率独立、速度加顶、精英随难度增长、开火地板、时间项软上限）、奖励随难度增长（含擦弹吃连击、迷雾存活补偿、事件奖励上调）、缓存正向回补与里程碑反馈。新增两条回归面：`--feel-probe`（顿帧/震动复位）与 `--long-probe`（难度曲线落在预期带，冒烟扩至八趟）。所有玩家可感取值按建议值落地并经人类决断**定稿 + 实机核验通过**（口径见 DESIGN_BASELINE §1.4.1/§2.9）。原先记为「未做」的三项（必死曲线收束口径与命名档位、受击喘息口径、Boss 攻击密度接 D）已在 `12f8350`/`0f8e11d` 落地；随后一轮严格审查修掉两条假绿回归面与两处实现缺陷（`95d5283`/`533b7d2`）。为什么：审查判定现状把弹幕游戏做成了只靠数值膨胀的无尽曲线，且命中无顿帧、震动非 trauma 模型是「观感不尽人意」的机制级成因。反转＝人类改值或推翻方向。
