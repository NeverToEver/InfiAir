...
- **[低] 探针体系覆盖缺口（2026-09-12 事件状态机审计发现，收口挂探针宿主化）**：`--event-probe` 经 `TryStartEncounter` 直启事件，绕过 `Eligible/Advance/CanTrigger/CanDriveEncounters` 整条生产判定链——生产触发断线（分数源读错、门控恒 false）时探针照绿；且探针在入场动画期（spawner 停驱动、生产不可能触发）启动事件，覆盖的是生产不存在的状态。另两处同根：精英探针的绿依赖「无头局玩家 30s 内不死」的数值巧合（死亡路径探针永覆盖不到）；编队锚点 x0 取 `GD.RandRange` 默认序列（`FormationStrikeEvent.cs:242`），违反 §5 确定性第 3 条。为什么不现在做：生产触发的回归面要等探针宿主化后才有落点。收口条件＝探针宿主化收口时一并：生产触发链接线入无头回归、锚点改可注入取值源。触发时机＝探针宿主化动工。
- **[低] 待确认：编队「全数拦截」档口径（默认已按可达口径落地）**：结论：建议全数拦截＝投出全拦（`_dropped>0 && _intercepted==_dropped`）+ 一架未坠（`_alive==_total`）。依据：原口径「一架未坠 + 一枚未投」恒不可达（收场三路径可推出存活机的投弹点必然全部到点投出），`reward_intercept`(400) 与专属台词是死代码，最难操作落进全歼档只拿 200——收益倒挂。代价：新口径下「拦截全部炸弹且不打编队机」即可拿最高档，比原字面口径（疑似本意不可达）容易。收口条件＝人类认可或改口径；认可后同步 DESIGN_BASELINE §1.8 的「一枚未投」表述。
- **[低] 待确认：弹反成功但反射弹未命中编队机不计拦截（仅提议，未改）**：`FormationBomb.cs:266-269` 反射弹出界回收不计 `_intercepted`、不发逐枚拦截分；事件侧 docstring 写「被空中击落/弹反」即奖，弹侧写「弹反命中」才算——同一事实两种口径。弹反已把炸弹威胁拆掉（反射上行），不计回报是否合设计意图归人。收口条件＝人类认可现状或改计；触发时机＝下次触摸 FormationBomb 拦截语义。
