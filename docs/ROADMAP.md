...
- **2026-09-10 视觉全面升级（战术琥珀）**（用户指令「参照死亡搁浅/合金装备/艾尔登法环/Apex 的科技感与交互感，全面升级 UI 与外观，去除原始低廉感，且不误伤操作逻辑」）：配色方向 = 战术琥珀（深炭蓝黑底 + 琥珀主交互 + 全息青降为数据通道 + 金=稀有 + 红=危险）；渲染 = 世界层后处理（GL Compatibility 下 `Environment` 辉光不可用，故手写 `world_grade.gdshader` 屏幕纹理柔光+暖调分级+晕影+颗粒，默认开、设置可关、`reduce_flash` 联动）；资源 = 重制玩家机（画布 254×254 与全部锚点冻结、`PlayerAugmentVisuals` 叠加件仍贴合）+ 战斗场景观感 + UI 全站，敌/精英/Boss/母舰贴图保持晶体棱镜身份仅运行时增强。**硬约束：玩法判定零改动**（碰撞半径/Hitbox/擦弹/宽限/弹速/伤害/Boss 相位/难度曲线全不触碰）。口径登记见 `DESIGN_BASELINE §2`；数值 `balance.json effects.world_post.*`；门禁 = 构建零警告 + 导入无警告 + 300 帧冒烟 + 窗口化人工过目。
- **2026-09-10 过场演出细节精修**（用户指令「简单的地方够简单了，该精细的地方——宇航员外观、机械仪表盘、场景细节——却不够，细节充斥能用就行的感觉」）：新增共享乘员构件 `CrewFigure.cs`（分件头盔/胸甲/维生背包/关节护件/手套靴底，关节契约逐位不变）替换开场镜头 3 与返航三镜头的两处重复简笔人物；镜头 4 操作台补圆形读数表/电平条/面板浮雕；`DawnStation` 舱段补舷窗/分缝/端盖/格栅；镜头 2 X 光剖面补舱内设备；镜头 5 弹射通道补导轨/轨枕/加强肋。**纯表现层，时序/时长/字幕/音效口径不变**；口径见 `DESIGN_BASELINE §2.4`。
- **2026-09-10 本局存档边界修复（检查点模型）**（用户核实读档边界后指令「按最优办法全量修复」）：确立**存档 = 可继续的检查点**三条规则——① 写入/覆盖（保存退出 + 回基地自动存）；② **终结删档**（死亡 + **`RestartRun` 放弃重开**；后者为本次修复补上，堵住「读档→暂停重开→再退出重进」的**无限回滚**）；③ **非破坏性**（标题屏选「新的一局」**不再删旧档**，防一次误触抹掉进度；`ExitToTitle` 亦不删——Tutorial 走此口会误伤，且回标题保留检查点语义正确）。另修：`HasRunSave()` 由「只判文件存在」改为**校验可读性**（文件 + `version` 匹配，损坏档触发既有隔离并返回 false），消除损坏/版本不符档仍显示「继续」提示的**假承诺**。口径见 `DESIGN_BASELINE §2.5`。
- **2026-09-10 本局存档（反转 2026-09-08「无对局存档」）**（用户报告实际缺口：退出确认称「将丢失本局进度」，但进入游戏无从读取；指令追加「进游戏可选择是否加载上次存档」「退出可选择不保存/保存并退出」）：新增单存档位 `user://run.json`（复用 SaveManager/SaveStore 原子写+损坏隔离，与 settings.json 分区）。标题屏存在可读存档时显示 `C — 继续上次出击`；战斗中退出确认由单「确认退出」改三键（保存并退出 / 不保存退出 / 取消）。新增各服务 `RestoreRunState`（含 `TalentCache.Snapshot/RestoreValues`）。**坑**：JSON 往返把 `StringName` 键退化为 `String`，还原时须归一化 `augments`/`missions` 键，否则查表落空、增幅与任务进度静默失效。口径见 `DESIGN_BASELINE §2.5`。
- **2026-09-10 性能设置（帧率上限 + 垂直同步）**（用户指令「加入有关于性能的控制，可参照主流游戏…可调整帧数，在 60 基础上追加 120/144/165/180/240 档位可调」）：设置页「操作模式」页新增**性能**段——帧率上限六档（60/120/144/165/180/240，默认 60，写 `Engine.MaxFps`；项目此前无任何帧率上限/VSync，实测未锁帧约 680fps）+ 垂直同步开关（默认开，写 `DisplayServer.WindowSetVsyncMode`）。`GameState._Ready` 加载后显式 `ApplyDisplaySettings()` 补默认档；切换即时生效+落盘+广播 `DisplaySettingsChanged`。口径见 `DESIGN_BASELINE §2.6`。
