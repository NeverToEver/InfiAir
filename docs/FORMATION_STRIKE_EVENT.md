...
事件短（约 12s）、不暂停普通波次、不冻结 Boss 调度。重制版新增内容（原作无此机制，不计入移植差距清单）。
...
- **不冻结 Boss 调度、不暂停普通波次**：事件运行期间波次照常刷怪，Boss 到期照常触发（警告+入场约 2s+，
  此时编队已近离场，炸弹有预警圈可躲，叠加风险可控）。
- **可被返航打断**：`Main._start_homecoming()` 调用事件 `abort()`（同母舰收回语义），编队立即解散离场，无结算；已投放的炸弹为独立实体，自然存续（语义同敌弹）。
