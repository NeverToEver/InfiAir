...
### 4.10 机体程序化动画调研（2026-09-19）

| 惯例 | 本项目现状 |
| --- | --- |
| **动画十二原则对程序化运动同样适用**：squash & stretch（重量与弹性——「a ball that squashes on impact and stretches as it moves feels physical and alive, while a rigid one feels dead」）、follow-through / overlapping action（次级元素滞后于主运动）、timing（缓动传达重量）；「an object that squashes on impact and has some follow-through feels physical where a rigid one feels cheap」 | 本轮落地（DESIGN_BASELINE §2.16）：冲刺缩放过冲、运动滞后漂移、转向跟随角惯性——全部程序化、只写贴图变换 |
| **Sakurai Motion 系列**：「Overdoing it is just right」（小屏幕上「不足」是常态失败）；「The follow-through decides the impression」；「Idle is the origin of all. Don't waver it」——基准位要稳；**响应优先于粘滞**（Smash 优先即时响应，「carefully interpolating from idle increases a sticky feel」） | 姿态/活性全部只作用于贴图层（判定/操控/弹道零滞后）；浮动幅度压到 1.6px（活着不成晃）；过冲只做一次性事件（冲刺起手）不做持续粘滞 |
| **Swink 三层模型**（§4.8 已引）：实时控制是 polish 的前提——滞后只能放表现层 | 同上：机体根节点与碰撞体不动是本批硬约束 |

来源：`bugnet.io/blog/animation-principles-every-game-developer-should-know`（2026-06）、`en.senkohome.com/sakurai-game-dev-motion/`（Sakurai「Motion」18 期按主题重构；原频道 `youtube.com/@sora_sakurai_en`）（检索入口：DuckDuckGo 实浏览器检索，2026-09-19；GitHub `grapefrukt/juice-it-or-lose-it` 已 404 且未见可用镜像，Juice 讲稿要点由 §4.8 的二手综述间接覆盖）

### 4.11 机身反馈与损伤状态调研（2026-09-19）

| 惯例 | 本项目现状 |
| --- | --- |
| **RCS / 微调推力器按所需方向点火**（真实航天器惯例）：「Reaction control systems are capable of providing small amounts of thrust in **any desired direction** or combination of directions」；常用大小推力器组合（vernier）分级响应 | 本轮落地（DESIGN_BASELINE §2.17）：左右/机首三枚小喷口，按机体本地系加速度**反向**点亮（推力与加速度反向的物理读数），余量随该轴加速度占比 |
| **状态可读性**（§4.7 已引 The Level Design Book：动画/细节要 telegraph state）与「最亮＝最危险」（§2.12 判据 3） | 损伤状态（重伤档）改为**可见的生命读数**：损伤烟 + 引擎喘振；开火机身光用暖向提亮且 ~0.2s 衰减，不构成新的亮度峰值语义 |
| **muzzle flash 让每次射击「violent and immediate」**（§4.8 已引 Vlambeer 手法表） | 既有枪口辉光 + §2.17 新增机身火光：光照落在机体自身，射击的重量不止在枪口 |

来源：`en.wikipedia.org/wiki/Reaction_control_system`（2026-09 实读；引文取自 Uses / Location of thrusters 两节）；其余两条为既有 §4.7/§4.8 条目的延伸适用（不新增出处）
...
**玩法与无障碍**：`hardcoregaming101.net/battle-garegga`、`.../dodonpachi`、`.../touhou`、`gdcvault.com/play/1023470`、`hardingfpa.com`、`gameaccessibilityguidelines.com/{basic,intermediate,advanced}`、`learn.microsoft.com/en-us/gaming/accessibility/guidelines` 与 `.../xbox-accessibility-guidelines/{103,108,110,116,117,118}`、`accessible.games/accessible-player-experiences/`、`egmatic.com/blog/how-to-make-your-game-feel-good`、`en.senkohome.com/sakurai-game-dev-specification/`、`en.senkohome.com/sakurai-game-dev-motion/`、`youtube.com/@sora_sakurai_en`、`bugnet.io/blog/animation-principles-every-game-developer-should-know`、`gamedesignskills.com/game-design/game-feel/`、`en.wikipedia.org/wiki/Reaction_control_system`
