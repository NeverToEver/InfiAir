...
## 6. 改进落实状态（2026-08-02 更新）

> 本报告各项建议的落地与处置登记；代码级改动均带对应提交与测试验证。

| 编号 | 建议 | 状态 | 处置 |
| --- | --- | --- | --- |
| P0-1 | 手柄支持实机验证与完善 | ✅ 已落地（代码层） | 手柄默认绑定运行时装配（左摇杆移动/动作键/右摇杆虚拟准星瞄准），`base_system_test` 3 断言；**实机手感验证仍需硬件**（登记为发布前验证项） |
| P0-2 | 高密度弹幕敌我可读性 | ✅ 已落地 | 玩家弹白芯高光描边（金边白芯），敌弹隐藏；敌弹密度动态降档经评估缓行（热路径复杂度 vs 收益） |
| P0-3 | 局外进度感（本地排行榜） | ✅ 已落地 | GameState 榜单数据层（上限 10/判型/持久化）+ 结算页本局排名与 Top5 + 开始页 Top3；`base_system_test` 14 断言 |
| P1-4 | 难度公平感审计 | 📋 审计结论 | 机制完备：Boss telegraph 前摇 0.35s/0.2s、P2 阈值 0.7、狂暴 0.3、出生/受击无敌、必死曲线可预期、敌机 4 型+精英 3 型有限组合；**高 ramp 段随机组合不可避性需实机 15 分钟+ 长局验证**（登记为验证项） |
| P1-5 | 音频辨识度与循环核对 | 📋 核对通过 | BGM 只设 `LOOP_FORWARD`（无 loop_begin/end 泄漏风险）、程序化生成 40s 无缝循环（平移周期化）、无 `_exit_tree` 停 BGM 违规；动态分层增强缓行 |
| P1-6 | 教程可重看 | ✅ 已落地 | 通关后无存档时教程按钮放行（E02/G03 禁用条件收窄为仅有存档）；`startup_flow_test` 断言更新 |
| P2-7 | 存档可见性说明 | ✅ 已落地 | README 中英补存档位置与自动保存行为说明 |
| P2-8 | 第二外语（俄语） | ⏸ 暂缓 | 需高质量人工翻译，AI 直翻入库质量风险高；若进入俄区市场再评估（`translations.csv` 已支持多列） |
| P2-9 | 波次节奏评估 | 📋 审计结论 | 节奏机制参数化完备（`spawner` 波次间隔 ramp/休整波/事件插入）；固定视口无卷轴，节奏靠敌潮密度与事件维持，与差评"卷轴慢"问题不直接对应；微调空间留配置 |
| P2-10 | 致死弹来源高亮 | ✅ 已落地 | 命中致死时弹丸停位移/关碰撞/红闪残留 0.5s，配合 Meta HUD 定向波纹；Timer 随场景树释放 |

**提交记录**：`9bc735d`（P0-2）→ `40716c4`（P0-3）→ `bb1d027`（P1-6）→ `275d719`（P2-10）→ `bcc5ace`（P0-1）→ 本文档归档提交（P1-4/P1-5/P2-7/P2-9 登记）。

**发布前验证项（需硬件/实机，已登记）**：① 手柄全流程实机走查（含双摇杆手感）；② 15 分钟+ 长局公平感实机验证；③ BGM 循环听感人工核对。
