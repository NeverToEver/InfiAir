...
**执行状态**（2026-07-24 对照代码复核）：P0/P1 五项已全部修复（提交 `4df9e02`，2026-08-02 口径统一订正——原记 `b02be46` 为失效哈希，实际落地提交为 `4df9e02`「审计 P0/P1 五项修复」），验证断言已落地，复跑 0 FAIL；P2 已完成 2 项，其余待办（行号为复核时现状）。

> **后续跟进（2026-08-02 口径统一）**：本文为历史执行记录（2026-07-24 时点）。P2 剩余待办多数已被后续审计轮次覆盖处置（如对象池 `_exit_tree` 清注册由 C21 落地、`profile_corrupt` 提示消费见 D 系列审核），仍在开放项的当前处置见 `docs/DESIGN_BASELINE.md` §7.3 与 `docs/AUDIT_VAULT.md`。
...
### 已完成
- `ui_theme.gd make_panel_style()` — 已随 UI 重构（`7f0aa42`「UI 样式与处理逻辑重构」，2026-08-02 口径统一订正——原记 `57c778b` 为失效哈希）删除。
