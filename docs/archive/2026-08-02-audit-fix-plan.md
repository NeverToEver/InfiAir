...
> **处置状态（2026-08-02 口径统一回填）**：✅ 已完成——代码修复提交 `a4318ea`（D 系列 P1/P2/P3 修复），文档同步提交 `f78363e`（D21-D28 等）；Boss P2 走位（D05）另经 `4a7d252` 落地。全量回归 30 断言场景 0 FAIL（2026-08-02 订正：entry_animation_test 已存在，实为 30）。
> 审查基线：`8c6dfff`（2026-07-29 波次化刷怪）→ `HEAD`（2026-08-02，59 提交、196 文件、+14.6k/-3.9k 行，按修复提交 `a4318ea` 时点实测，2026-08-02 口径统一订正）。
...
- 判定：**登记不修（待作者确认）**。设计确认类：改行为属产品级改动，超出本轮范围；回写 BOSS_REDESIGN §8 自决点登记差距，防止后人按文档误判现状。若作者确认为遗漏，后续按表格补实现。——**后续（2026-08-02 口径统一）**：确认为遗漏，已于同日经 Boss P2 走位计划实现（提交 `4a7d252`，见 `docs/2026-08-02-boss-p2-movement-plan.md` 与 AUDIT_VAULT D05 修复起效记录）。
- 验证：文档核对。
...
### 判定不修（登记，不产生代码改动）
D11、D15、D16、D19、D20、D30；D05/D18 仅文档登记。——**2026-08-02 订正**：D05 后经 Boss P2 走位计划实现（`4a7d252`），D18 为 RETURN_HOME_CINEMATIC §9 音频口径说明（文档核对项）。
...
**全量回归（修复后）**：30 断言场景 0 FAIL（2026-08-02 口径统一订正，原记 29 系漏计 entry_animation_test）+ perf_bench rc=0 + `--quit-after 300` 0 error；autoplay 探针完整跑（480s、3 对局、0 死亡）：孤儿节点 0、帧耗时峰值 7.43ms（基线 6.90ms），**1 个 score_stagnant 偶发**——Boss 战专注期（玩家血量 43 龟缩躲弹）分数 60s 停滞 + Boss 逃跑空窗瞬间触发；该 run 返航 0 次、异常点与 D 系列改动路径（返航/入场/Timer 清理）无交集，判定为既有探针偶发（同 smoke 偶发基线），非本次改动引入。
