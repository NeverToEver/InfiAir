...
> 状态：**审核完成，只登记未修复**。发现登记见 `docs/AUDIT_VAULT.md` G 系列（第五轮）；本文件为审核范围、规则、证据与修复优先级追踪单一事实源。
...
**建议修复优先级**：
1. **P1 立即修**：G01（Boss 预警返航整局空转）、G02（Boss 逃跑期补刀奖励失真）、G03（教程入口静默删档）
2. **P2 本批顺手**：G04（改键冲突）、G08（硬编码 -280）、G09（敌弹每发 JSON 查询）、G011（进度条残留）、G05/G06（热路径/判型）
3. **P3 累积待办**：按上表判定建议，优先 G013（存档层数钳制）、G014（教程坐标）、G016（信号断开）、G029（cfg 类型）
4. **待判定项**（G07/G010/G018-G020/G023/G025/G026）需产品/架构决策，建议下批专项评审

> 注：D01 审计记录宣称「Boss 预警取消后按门控再触发」实际因 G01 的 `_boss_active` 卡死而不成立，修复 G01 后该口径自然恢复。
