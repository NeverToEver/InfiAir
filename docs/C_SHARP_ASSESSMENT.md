...
| 工程化收益 | 有限。现有 5 层门禁（gdformat + gdlint + warning-as-error + 冒烟 + 41 断言场景）已覆盖 C# 静态类型可防的大部分错误类别 |
...
- 唯一 autoload：`GameState`（facade，转发 4 个非 autoload 服务：balance/save/sfx/entity_registry）。
- 弹幕热路径已充分优化：`BulletPool` 对象池复用 + 同屏敌弹 500 硬上限（`bullet_pool.gd`）、`BossFire` 纯发射逻辑（RefCounted，A3 拆分）、策略/波次均由配置驱动（`data/balance.json`）。
- 架构债务清零：A2 服务拆分、A3/A4 注册表 + 声明式效果表、A8 PlayerVisuals 拆分全部落地；41 断言场景 0 FAIL（2026-08-04）。
...
- **CI**（`.github/workflows/ci.yml`）：官方 Godot 4.6.2 **标准版** headless（非 .NET 版）→ gdlint/gdformat 门禁 → 无头导入（warning-as-error）→ 主场景冒烟 → 41 断言场景全量 + 编译探针。
...
- 41 断言场景 + autoplay + perf_bench 全为 GDScript，CI 中可继续跑；但任何 C# 代码新增 `dotnet build` 前置步骤，测试矩阵复杂度上升。
...
5. **既有工程化保障足够**：warning-as-error + gdlint + 41 断言场景已把 GDScript 的短板（弱类型）压缩到低风险区间（§4.2）。
