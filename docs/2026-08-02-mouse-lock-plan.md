...
> 状态：**已完成**。审计登记见 `docs/AUDIT_VAULT.md` F 系列（F01）；本文件为发现-设计-落地-验证追踪单一事实源。
...
- **headless**：`Window.mouse_position` 在 headless 显示服务器不可访问，`_process` 开头按 `DisplayServer.get_name()` 跳过。
...
| （C4 文档） | AGENTS/ARCHITECTURE/DESIGN_BASELINE/TESTING/AUDIT_VAULT + 本文件 | — |
