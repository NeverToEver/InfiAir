...
5. **推送并开 PR**：PR 会自动触发 GitHub Actions CI（无头导入 + 主场景冒烟 + 31 断言场景全量回归），**CI 全绿是合入门槛**。
...
- 31 个断言场景（`test/*_test.tscn` 中除 `autoplay_test` 的探针外）+ `autoplay_test`（长时异常探针，本地按需跑）+ `perf_bench`（性能基准，需 `--fixed-fps 1000`）+ 7 个窗口模式截图工具。
