...
public partial class Main : Node2D
{
    ...
    /// 在 _EnterTree 注入）。只关掉标题屏交接与回基地自动存档，其余全按正局走——本局时钟、
    /// 事件触发、结算页、回基地都照生产路径（练习是生产模式，不是宿主驱动）。</summary>
    private bool _practice;
    /// <summary>_Ready 是否已跑完：<see cref="MarkPracticeRun"/> 的护栏判据——注入晚于它时
    ...
    public override void _Ready()
    {
        ...
        // 迷雾事件：本局自动触发（首延迟过后每 3s 掷签）
        var fogV = GameState.Instance.FogEvents;
        _fogEvents = fogV;
        _fogEvents.SetRunActive(true);
        ...
        else if (GameState.Instance.BootHandoffDone)
        {
            ...
            //
            // 本局存档：标题屏选「继续上次出击」→ PendingLoadRun 置位，读档成功则
    ...
    /// 因此只关掉「交接标题屏」与「回基地自动存档」，其余生产路径逐条照走
    /// （本局时钟/遭遇与迷雾触发/结算页/直选 Boss 与遭遇的请求）。
    /// 时序约束：必须在 <see cref="_Ready"/> 之前调用（_EnterTree 由父到子），
    ...
    public override void _Process(double delta)
    {
        ...
        // DYING 呼吸缩放：仅激活期逐帧组合；退出激活时复位一次到基础 zoom
        ...
        // 死亡回放录制（每渲染帧采样敌弹轨迹；死亡后树暂停本函数不再执行）
        // 数据源为敌弹注册表，消除每帧 get_children + cast 分配链
        _replay.Record();
    }
    ...
    private void BuildChargeFx()
    {
        ...
        // 与虚影同位（复用 _charge_ghost.position.x，不写死 960）
    ...
    private void OnReturnFinished()
    {
        ...
        //
        // 练习局不落盘：练习与检查点无关（权威守卫在 GameState.SaveRun，
        // 这里显式跳过是为了不留一条合法路径的噪声日志）。
        if (!_practice && !GameState.Instance.SaveRun())
        {
    ...
    /// 具名无参回调替代闭包捕获——闭包无法配对断开，Boss 若改对象池复用即双订阅；
    ...
    {
        // 0.1s 轮询缓存——分支/取整参数/语言未变直接复用上次结果
    ...
    {
        // mothership_recall 天赋：每级冷却 ×0.75（60s→45s→34s）；浮点有效层级（收益递减折算）
        _dockCooldown = cooldown
            * Mathf.Pow(
                (float)GameState.Instance.Cfg("augments.mothership_recall.cooldown_factor", 0.75).AsDouble(),
    ...
    private void StartHomecomingInternal()
    {
        ...
        _fogEvents.EndActive();
        // 返航统一清除召唤/对接期残留的 999s 无敌——_summon_mothership 设 set_invincible(999.0)，
        ...
        _player.AbortEntry(); // 入场中按 B 返航时复位入场状态机（防新入场被守卫跳过）
    ...
    private void StartEntrySequenceInternal()
    {
        ...
        {
            _player.EntryFinished += OnEntryFinished; // C# [Signal] 以 PascalCase 注册
