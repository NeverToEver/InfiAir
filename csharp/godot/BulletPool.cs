...
{
    // 静态 PackedScene 持有 → 实例字段（禁静态持 Godot RefCounted）
    ...
    /// <summary>待停放队列——Release 入队，_Process（idle 帧，物理回调外）批量
    ...
    /// <summary>帧末批量停放（idle _Process 处于物理回调外，与 CallDeferred 同帧末语义）。
    /// IsActive 仲裁——同帧重激活的弹跳过；
    /// 待帧末删除的弹跳过。</summary>
