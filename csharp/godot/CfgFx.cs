...
/// <summary>
/// 判型口径统一助手：Cfg 标量键读取 + 类型守卫 + 可选域钳
/// （对齐 Enemy.cs 同款判型）——PathResolver 已保证数值键坏类型回退默认，此处判型为
/// 第二道防线（坏类型 AsDouble/AsInt64 抛 InvalidCastException 崩溃）；仅 _Ready 一次性
/// 调用非热路径。min/max 与调用点 Mathf.Max/Mathf.Clamp 参数逐一对应，不得新增/删除钳制。
/// </summary>
public static class CfgFx
{
    /// <summary>时长/间隔类配置的统一钳制下限：≤0 使计时器每帧触发（攻击风暴）
