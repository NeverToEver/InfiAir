...
public partial class LaserWeapon : Node2D
{
    ...
    /// <summary>冷却时长（s），_ready 从 balance.json 覆盖。</summary>
    ...
    public override void _Ready()
    {
        ...
        // tick_damage 钳 ≥1（long 域比较再 (int) 防回绕）——≤0 经 EntityDamage.Dispatch 给敌机回血
        TickDamage = (int)Mathf.Max(GameState.Instance.Cfg("augments.laser_beam.tick_damage", TickDamage).AsInt64(), 1L);
        // length/half_width/hit_radius 钳 0.1 下限——≤0 光束线段退化永不命中；hit_radius 先钳再乘 WorldScale
    ...
    /// <summary>光束方向：经 _player.AimPoint()（磁吸/粘性后的平滑瞄准点），与准星/开火共用同一点
    /// （磁吸会让准星偏离原始鼠标，仍走 get_global_mouse_position 则光束与可见准星指向不同目标）。</summary>
