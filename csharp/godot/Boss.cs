...
/// 4 月蚀），HP 分段驱动阶段框架：P1（100–70%）→ P2（70–30%）→ ENRAGE
...
/// 阶段转场清弹 + 玩家短暂无敌、狂暴锁血 30% + 玩家移速 ×0.35、逃跑警告 +
...
public partial class Boss : Area2D, IDamageable, IPushableDamage, ISlowable
{
    ...
    /// <summary>常规阶段：P1/P2 模式表循环，ENRAGE 为狂暴（序列结束后「余怒」沿用 P2 表提速）。</summary>
    ...
    /// <summary>「余怒」倍率：射速 ×1.3（原 ×1.5 下调）/ 移速 ×1.3。</summary>
    public float EnrageRateMult { get; set; } = 1.3f;
    public float EnrageSpeedMult { get; set; } = 1.3f;
    /// <summary>狂暴期玩家减速乘区（替代定身）：TRANSITION+ACTIVE 期间移速 ×0.35。</summary>
    public float EnragePlayerSlow { get; set; } = 0.35f;
    /// <summary>段切换演出时长（蓄力辉光 + 停火）。</summary>
    ...
    /// <summary>狙击 telegraph：瞄准线 0.35s（前 0.2s 微跟踪玩家后固定），到点沿线出弹。</summary>
    ...
    /// <summary>一型 P1 纵向下压：每 6s 下压 80px 再回。</summary>
    ...
    /// <summary>蓄力重炮（一型 P2）：0.6s 蓄力辉光 → 3 发高速重弹（间隔 0.25s，每发 0.15s 短闪光）。</summary>
    ...
    /// <summary>冲刺掠过（二型 P2）：0.5s 瞄准线 → 高速横穿玩家高度，路径拖 3 枚减速弹。</summary>
    ...
    /// <summary>编队齐射（三型 P2）：召唤 4 小怪列横队，0.8s 后齐射一轮自机狙。</summary>
    ...
    /// <summary>弹幕墙（三型 P2）：10 路低速扇形墙，留 2 个相邻缺口（缺口方位避开自机 ±30°）。</summary>
    ...
    /// <summary>
    /// 难度分档（boss.difficulty_scaling）：索引 = [easy, medium, hard]。
    ...
    public Godot.Collections.Dictionary DiffCountDeltas { get; set; } = new()
    {
        ...
        // ring_burst 为绝对值分档（json 缺键时回退此表）
    ...
    /// <summary>一型狂暴「旋转堡垒」（boss.enrage.type_1）。</summary>
    ...
    /// <summary>二型狂暴「猎杀环绕」（boss.enrage.type_2）。</summary>
    ...
    /// <summary>三型狂暴「倾巢」（boss.enrage.type_3）。</summary>
    ...
    /// <summary>血条下方逃跑倒计时显示起点（剩余 ≤10s）。</summary>
    ...
    private void LoadBalance()
    {
        ...
        // 保序：P2 段必须高于 ENRAGE 线（70%→30% 顺序），倒挂配置
    ...
    /// <summary>逃跑剩余秒数（HUD 逃跑倒计时读取口）。</summary>
    ...
    /// <summary>P1→P2 段切换：0.6s 蓄力辉光 + 抖屏 + 变调音效 + 清自身开火计时，模式表重置循环。</summary>
    ...
    /// <summary>
    /// 难度分档统一应用：档位 = GameState.difficulty（easy/medium/hard → 索引 0/1/2），
    ...
    private void ApplyDifficultyScaling()
    {
        ...
        // 三型普通阶段召唤间隔随难度分档（对齐「各内部节奏 ×interval_mult」）；
        ...
        // 4 型普通阶段 ring_burst 环弹速 + 狂暴双环/蓄力环阵弹速随难度档（对齐全弹速分档）
