...
public partial class EnemyPool : Node
{
    ...
    /// <summary>待回收/待激活回挂队列——Release/Spawn 入队，_Process（idle 帧，物理回调外）
    /// 批量执行，替代每敌 CallDeferred（ReparentDeferred/ReparentToActive）逐条消息派发。</summary>
        ...
        Godot.Collections.Dictionary config, StringName strategy, float pDifficulty, Vector2 pos, StringName pBulletType)
    {
        ...
        {
            // spawn 侧 reparent 在物理回调（碰撞信号）内触发 area_set_shape_disabled flush 报错，
            // 与 Release 侧停放对称入队，_Process 帧末批量回挂（替代 CallDeferred 逐条派发）
    ...
    /// 幂等守卫用 IsActive O(1)——Deactivate 必置 false（本方法是
    ...
    /// <summary>帧末批量停放/激活回挂（idle _Process 处于物理回调外，与 CallDeferred
    /// 同帧末语义）。两队列以 IsActive 双向仲裁（同 ReparentDeferred/ReparentToActive 互斥）：
    ...
    public override void _Process(double delta)
    {
        ...
        if (_pendingActivate.Count > 0)
        {
            for (var i = 0; i < _pendingActivate.Count; i++)
            {
                ...
                // reparent 触发 e._exit_tree，置位防 unbind_enemy 误发信号
                e.SetRepooling(true);
