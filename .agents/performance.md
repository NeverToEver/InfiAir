...
- 改池保持 `SetRepooling` 双向包裹（EnemyPool spawn/release、BulletPool release；否则 `_ExitTree` 误发 `EntityUnregistered` 或误清闲置池）；改池后跑 main smoke 300 帧 + `test/smoke_test.tscn`（原 `pool_reuse_test`/`entity_manager_test` 已于 2026-08-29 退役, 池语义回归靠 xUnit + 手动验证）。
