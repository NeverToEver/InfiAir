...
- Deps/output: launchers detect engine + version (Godot 4.6+): run.command 用 `version_ok` 候选选型（`version_ok` 函数在 run.command，run.sh 仅警告式检查）；run.bat 探测 `--version` <4.6 警告并保真退出码（R09：`endlocal & exit /b %EXIT_CODE%`，原 `if errorlevel 1 pause` 把退出码归零）；release.sh 的 `VERSION` 自动读取 `project.godot` `config/version`（sed，取不到回退 3.26），打包前 `command -v tar/zip` 前置检查。`command -v` external tools. Colors ok but respect `NO_COLOR`.
