...
- Corrupt JSON isolated as `<file>.corrupt`, notified to start screen via `save_corrupt`/`profile_corrupt`. Don't bypass recovery. Note (2026-08-06 audit): `save_corrupt`/`profile_corrupt` cover the run save and `profile.json` only; a corrupt `users.json` is silently rebuilt as an empty DB (`UserDB._ensure_loaded`), which from the player's perspective looks like accounts vanishing — do not change without surfacing it in the start screen flow first.
