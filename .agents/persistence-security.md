...
- Corrupt JSON isolated as `<file>.corrupt`, notified to start screen via `GameState.SaveCorrupt`/`ProfileCorrupt` flags. Don't bypass recovery. Note (2026-08-06 audit): `SaveCorrupt`/`ProfileCorrupt` cover the run save and `profile.json` only; a corrupt `users.json` is silently rebuilt as an empty DB (`UserDb.EnsureLoaded()` in `csharp/core/Storage/UserDb.cs`), which from the player's perspective looks like accounts vanishing — do not change without surfacing it in the start screen flow first.
