...
- Don't touch existing autoloads/input mappings for unrelated needs. Inputs (`project.godot`): `move_up`/`move_down`/`move_left`/`move_right` (WASD/arrows), `boost` (Shift), `fine_move` (Ctrl), `dash` (Space), `dock` (H), `homecoming` (B), `give_up` (K), `buff_panel` (L), `parry` (F, arcane shield, fairness mech #4), `restart` (R). Joypad defaults bound at runtime by `GameState.BindJoypadDefaults()` (keyboard only in project.godot; deadzone via `GameState.SetJoyDeadzone()`). PS detect via `GameState.IsPsGuid()` (vendor 054c; ✕○□△/L1-R1 labels).
