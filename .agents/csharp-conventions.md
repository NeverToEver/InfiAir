...
仓库为终态纯 C# 工程（零 GDScript，CI 零 GDScript 闸强制；迁移决策依据 `docs/archive/2026-08-08-csharp-assessment.md` §10）。`csharp/godot/*Interop.cs` 为 InfiAir.Core 的 C# 绑定端点，非 GDScript 壳。
...
- **静态审查工具**:Roslynator CLI 本地留存 `tools/roslynator/`(gitignored;重建:`dotnet tool install --tool-path tools/roslynator roslynator.dotnet.cli`),运行需 `DOTNET_ROOT=~/.dotnet`。`roslynator analyze <csproj>` 报告为 info 级建议、非 CI 门禁;应用口径(2026-08-10 AA 系列):CA1854/1846/1866/1869/1861/1859 安全子集可落地,**CA1822(标 static)不应用**——Godot 场景/信号按名连接对 static 方法有运行期解析风险。
...
- 热路径(对象池/弹幕/每帧循环)遵守每帧零托管分配红线(见 §工程红线)。
...
- 迁移期批次纪律与 P0/P1 渐进着陆点记录已随 M7 全量迁移完成失效,原文见 git 历史(2026-08-10 前版本)与 `docs/AUDIT_VAULT.md` U 系列。
- **P2-1 AssetCatalog 资产清单管理**(未启动,按需):触发条件 = 资产加载分散管理成本上升;当前 preload 分散但规模小,暂不做。
- **明确不做**:EventManager 迁移 core(与 fog 效果层耦合深)、DDA 迁移(调用点深)、任何热路径迁移(边界禁令)。
