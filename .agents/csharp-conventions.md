...
## Overview
终态纯 C# 工程（零 GDScript；2026-08-29 起 CI 不再单设零 GDScript 闸, 新增 `.gd` 由 code review 把关；迁移决策依据 `docs/archive/2026-08-08-csharp-assessment.md` §10）。`csharp/godot/*Interop.cs` 为 InfiAir.Core 的 C# 绑定端点，非 GDScript 壳。
## 工程红线
...
- `.editorconfig` 管 C# 风格（4 空格缩进、file-scoped namespace、max_line_length 140）；`dotnet format --verify-no-changes` 三工程零 diff 为本地习惯检查（2026-08-29 起不再是 CI 门禁）。
...
- 热路径（对象池/弹幕/每帧循环）遵守每帧零托管分配红线（见 §工程红线）；新增 Core 能力须配 xUnit。
## Test Layering
- 纯逻辑 → xUnit（`dotnet test`，毫秒级）；场景/集成 → C# 脚本化断言场景（Node + `_Ready` 断言 + `TestExit.Quit(failures)` + 入口 try/catch 保证异常也非零退出）。2026-08-29 削减后仅存 `smoke_test` + `base_system_test` 两个断言场景——新场景级行为优先 xUnit 化纯逻辑 + 手动/截图验证；确需新增断言场景时先在 `docs/TESTING.md` Scene Counts 登记（计数单一权威; 规则见 `.agents/doc-sync.md`）。
## Reference Sample
首个 Core 样板: `csharp/core/BalanceModels.cs` + xUnit `tests-csharp/BalanceModelsTests.cs`，新 Core 代码以此为参照（引擎侧绑定端点参照 `csharp/godot/BalanceInterop.cs`）。
