...
- **CI/CD:** `.github/workflows/ci.yml` (headless import + main smoke + 45 assertion scenes + BALANCE_MAP 生成器重跑零 diff 闸（M8，2026-08-06）; push/PR). Release: `export_presets.cfg` + `release.sh` → GitHub Releases (not in repo); manual `.github/workflows/release.yml` (export → tag `v<ver>` → release, syncs `config/version`). CI/CD changes sync these entry docs + `release.sh`; no 3rd-party deps beyond official checkout action + Godot binary/templates.
