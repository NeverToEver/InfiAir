...
- 不要修改既有 autoload 或输入映射来完成无关需求。现有输入由 `project.godot` 定义，包括移动、`boost`（Shift）、`fine_move`（Ctrl）、`dash`（Space）、`dock`（H）、`homecoming`（B）、`give_up`（K）、`buff_panel`（L，展开/收起 buff 滚动栏）、`parry`（F，弧光弹反盾，2026-08-03 公平感机制四）和 `restart`（R）。**手柄默认绑定（左摇杆移动/动作键/右摇杆瞄准/LT 弹反盾）由 `GameState._bind_joypad_defaults()` 启动时经 InputMap 运行时装配**（P0-1：`project.godot` 只存键盘，手柄不落 project.godot；死区经 `GameState.set_joy_deadzone()` 应用到全部手柄动作）。**PS 手柄自动识别**（`GameState.is_ps_guid()` 按 SDL GUID vendor 054c 判定，位置一致仅标签对应 ✕○□△/L1-R1，设置页显示布局）。
