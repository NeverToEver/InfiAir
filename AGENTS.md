...
- **Static analysis:** Roslynator CLI 本地留存于 `tools/roslynator/`（启动器 stub 已入库,72KB;完整工具链 `dotnet tool install --tool-path tools/roslynator roslynator.dotnet.cli` 可重建）。用法 `DOTNET_ROOT=~/.dotnet tools/roslynator/roslynator analyze InfiAir.csproj`；应用口径见 `.agents/csharp-conventions.md` §Build & Gate。
