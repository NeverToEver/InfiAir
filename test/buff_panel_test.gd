...
func _ready() -> void:
	...
	# ---------- 1. 无 buff：标签隐藏，L 不展开 ----------
	_check(not hud._buff_tag.visible, "无 buff：收起态标签隐藏")
	...
	await get_tree().process_frame
	await get_tree().process_frame
	_check(hud._buff_dock.get_child_count() == 3, "3 个 buff：收起态 3 格")
	_check(hud._buff_tag.visible and hud._buff_tag.text == "增益 [L]", "3 个 buff：标签带 [L] 提示")
	...
	await get_tree().process_frame
	await get_tree().process_frame
	_check(hud._buff_dock.get_child_count() == 5, "6 个 buff：收起态 4 格 + 溢出格")
	_check(hud._buff_overflow_label != null and hud._buff_overflow_label.text == "+2", "溢出格计数 +2")

	# ---------- 4. L 展开滚动栏：全量明细 ----------
	await _press_l()
	_check(hud.is_buff_panel_open(), "按 L：滚动栏展开")
	_check(hud._buff_rows.get_child_count() == 6, "滚动栏 6 行明细")
	...
	await get_tree().process_frame
	_check(hud._buff_tag.text == "BUFFS [L]", "en：标签刷新")
	_check(hud._buff_panel_title.text == "Active Buffs", "en：滚动栏标题刷新")
