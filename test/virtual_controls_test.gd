...
func _ready() -> void:
	...
	_check(vc != null and vc is VirtualControls, "场景1: Main 挂载 VirtualControls 并经 GameState 转发")
	...
	# ---- 场景2：左摇杆 → move_* ----
	await _touch(0, true, VirtualControls.MOVE_CENTER)
	_check(not Input.is_action_pressed("move_right"), "场景2: 中心按下（死区内）不注入移动")
	await _drag(0, VirtualControls.MOVE_CENTER + Vector2(VirtualControls.MOVE_RADIUS, 0.0))
	...
	await _drag(0, VirtualControls.MOVE_CENTER + Vector2(0.0, -VirtualControls.MOVE_RADIUS))
	...
	# ---- 场景4：虚拟按钮 ----
	var dash_btn: Dictionary = VirtualControls.BUTTONS[&"dash"]
	...
	var parry_btn: Dictionary = VirtualControls.BUTTONS[&"parry"]
	...
	var boost_btn: Dictionary = VirtualControls.BUTTONS[&"boost"]
	...
	var fine_btn: Dictionary = VirtualControls.BUTTONS[&"fine_move"]
	...
	await _touch(6, true, VirtualControls.MOVE_CENTER + Vector2(VirtualControls.MOVE_RADIUS, 0.0))
	_check(not Input.is_action_pressed("move_right"), "场景5: 禁用后触摸注入不产生 move 状态")
	await _touch(6, false, Vector2.ZERO)
	var dash2: Dictionary = VirtualControls.BUTTONS[&"dash"]
