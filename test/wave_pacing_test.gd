...
func _ready() -> void:
	...
	spawner._wave_timer = 3.0
	spawner.set_process(true)
	await get_tree().create_timer(0.5).timeout
	_check(spawner._wave_timer == 3.0, "Boss 激活期间波次计时不推进")
	...
	spawner._wave_timer = 0.05
	spawner._waves_since_special = 0
	var gap := -1
	var prev := 0
	spawner.set_process(true)
	var t4 := 0.0
	while gap < 0 and t4 < 10.0:
		...
		var cur: int = spawner._waves_since_special
		if prev > 0 and cur == 0:
			gap = prev
		prev = cur
	...
	_check(spawner._waves_since_special == 0, "精英波后特殊槽计数归零")
	for e in _enemies():
		if is_instance_valid(e):
			e.queue_free()
	await get_tree().process_frame
	...
	spawner._on_special_killed()
	_check(spawner._waves_since_special == -spawner.REST_WAVES_AFTER_KILL, "精英击杀后进入休整波次")
	spawner._waves_since_special = 0
	spawner._on_boss_died()
	_check(spawner._waves_since_special == -spawner.REST_WAVES_AFTER_KILL, "Boss 击杀后进入休整波次")
