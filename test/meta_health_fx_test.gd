...
func _ready() -> void:
	...
	_check(fx.hit_pulse() <= 0.151, "2：连续 10 次 r=0.05 伤害 _hit_pulse 不累积（≤0.15）")
	...
	for i in 18:  # 1.8s 观测窗（覆盖末次跨阈值后的 0.7s 消散全程）
		...
		max_jitter = maxf(max_jitter, fx.heal_jitter())
	_check(max_jitter > 0.3, "4：修复期 _heal_jitter 经历 0→0.35 峰值")
	_check(fx.heal_jitter() < 0.02, "4：0.7s 全程后 _heal_jitter 回落 0")
	_check(fx.crack_progress() < 0.2, "4：上行修复后 crack_progress 回落")
