...
InfiAir 是**纯单机游戏**：无网络通信、无远程服务、无第三方账号体系、无在线密钥或远程凭据处理（本地账户口令经加盐 PBKDF2 变体散列，存于 `users.json`）。运行时唯一的外部交互是本地 `user://` 持久化（每用户存档 `savegame_<user>_<hash>.json`、用户表/设置/榜单 `users.json`；`profile.json` 仅未登录/兼容）与可选的离线数值编辑器（`scripts/tools/balance_editor.py`，仅监听 127.0.0.1）。因此攻击面极小，但存档/档案文件的健壮性（损坏隔离、类型守卫）与本地口令散列仍是本项目安全相关的关注点。
